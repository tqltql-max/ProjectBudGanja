...
function publishStaticAssets(root) {
  ...
  const inspecoesFeed = feed.filter((p) => p.category === 'inspecao');
  const { embedInspecoesFeedInPage, embedInspecoesFeedInPagesJson } = require('./embed-inspecoes-feed.js');
  embedInspecoesFeedInPage(root, inspecoesFeed);
  embedInspecoesFeedInPagesJson(root, inspecoesFeed);
