...
const { pickShareableInspections } = require('./inspecoes-share-feed.js');
...
function publishStaticAssets(root) {
  ...
  writeFileRetrySync(path.join(root, 'posts-public.json'), JSON.stringify(feed, null, 2), 'utf8');
  const shareFeed = pickShareableInspections(feed);
  writeFileRetrySync(path.join(root, 'inspecoes-share.json'), JSON.stringify(shareFeed, null, 2), 'utf8');
  ...
  const targets = [
    ...
    { id: 'objetos/index.html', placeholder: '<!-- EQUIPMENT_POSTS_PLACEHOLDER -->', category: 'equipamento' }
  ];
