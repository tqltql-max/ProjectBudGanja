...
function setUserSessionCookie(headers, token) {
  ...
  // Lax: o cookie precisa ir no redirect do Google OAuth. Strict descarta a sessão.
...
function clearOAuthStateCookie(headers) {
  return cookieHeader(OAUTH_STATE_COOKIE, '', 0, isSecureRequest(headers), 'Lax');
}
...
async function updateUserProfile(store, userId, payload) {
  // Cultivo already lives in SQL; migrate leftover profile fields before this wipe.
  const { migrateCultivoFromProfileIfNeeded } = require('./cultivo-user-service.js');
