...
async function paginateBrowse(html, token, maxVideos, parseItem) {
...
async function fetchAllChannelVideos(channelVideosUrl, maxVideos) {
  const cap = Number(maxVideos) > 0 ? Number(maxVideos) : MAX_VIDEOS;
...
async function fetchUploadsPlaylist(channelId, maxVideos) {
  if (!channelId || !/^UC[\w-]{20,}$/.test(channelId)) return [];
  ...
  const rest = await paginateBrowse(html, token, maxVideos, parseAnyVideo);
  const all = videos.concat(rest);
  const seen = new Set();
  return all.filter((v) => {
    if (seen.has(v.id)) return false;
...
async function buildChannelCatalogFromUrl(input, options) {
  ...
  let pageVideos = [];
  try {
    pageVideos = await fetchAllChannelVideos(videosUrl, options.maxVideos);
  } catch (e) {
    console.warn('Página /videos falhou:', e.message);
  }

  try {
    const shorts = await fetchAllChannelVideos(channelUrl + '/shorts', options.maxVideos);
    if (shorts.length) {
  ...
  if (pageVideos.length < 80) {
    try {
      const playlistVideos = await fetchUploadsPlaylist(channelId, options.maxVideos);
      if (playlistVideos.length > pageVideos.length) {
        console.log('Playlist de uploads:', playlistVideos.length, 'vídeos');
        pageVideos = playlistVideos;
      }
    } catch (e) {
