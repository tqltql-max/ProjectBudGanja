...
const googleAuthLimiter = createRateLimiter({ prefix: 'google-auth', windowMs: 15 * 60 * 1000, maxAttempts: 25 });
...
async function handleApiRequest(req, options) {
  ...
  try {
    ...
    const maxBody = url === '/api/upload' || url === '/api/admin/update-icons' || url === '/api/user/avatar'
      ? MAX_UPLOAD_BYTES
      : (url === '/api/cultivo/photo'
        ? MAX_CULTIVO_MEDIA_BODY_BYTES
        : (url === '/api/cultivo' ? MAX_CULTIVO_STATE_BODY_BYTES : MAX_BODY_BYTES));
    if (bodyRaw.length > maxBody) {
      return jsonResponse(413, {
        error: url === '/api/upload' || url === '/api/admin/update-icons'
          ? 'imagem muito grande (máx. ~6 MB)'
          : (url === '/api/user/avatar'
            ? 'imagem muito grande (máx. 2 MB). Reduza a foto e tente de novo.'
            : (url === '/api/cultivo/photo'
              ? 'mídia muito grande para envio'
              : (url === '/api/cultivo' ? 'diário demasiado grande para gravar de uma vez' : 'body too large')))
      });
    }
  } catch (e) {
  ...
  if (url === '/api/auth/google/start' && method === 'GET') {
    const authKey = getClientKey(req, headers);
  ...
  if (url === '/api/auth/google/callback' && method === 'GET') {
    ...
    const user = await upsertGoogleUser(store, googleUser, authMeta);
  ...
  if (url === '/api/user/avatar' && method === 'POST') {
    ...
    try {
      ...
      const m = data.match(/^data:(image\/(?:png|jpeg|jpg|pjpeg|webp));base64,(.+)$/i);
