...
const { getInfraPlanForUser, updateInfraPlanForUser, publicInfraPlanView } = require('./infra-plan-service.js');
const { handleGoogleAuth } = require('./auth-google-oauth.js');
const { userImagesPrefix } = require('./user-images-service.js');
...
const cultivoPhotoLimiter = createRateLimiter({ prefix: 'cultivo-photo', windowMs: 60 * 60 * 1000, maxAttempts: 40 });
const cultivoWriteLimiter = createRateLimiter({ prefix: 'cultivo-write', windowMs: 15 * 60 * 1000, maxAttempts: 80 });
const infraPlanWriteLimiter = createRateLimiter({ prefix: 'infra-plan-write', windowMs: 15 * 60 * 1000, maxAttempts: 80 });
...
function getClientIp(headers, req) {
  ...
  const reqIp = String(req && (req.ip || req.remoteAddress) || '').trim();
  return reqIp.slice(0, 120);
}
...
async function exportStaticFiles(root, store, options) {
  ...
  if (pages && !skipPages) {
    for (const entry of PAGE_REGISTRY) {
      ...
      } else if (entry.file === 'objetos/index.html' || entry.file === 'equipamentos/index.html') {
...
async function handleApiRequest(req, options) {
  ...
  const authMeta = {
    ipAddress: getClientIp(headers, req),
  ...
  try {
    ...
    let maxBody = MAX_BODY_BYTES;
    if (url === '/api/admin/update-icons') maxBody = MAX_ICONS_BODY_BYTES;
    else if (url === '/api/upload') maxBody = MAX_UPLOAD_BYTES;
    else if (url === '/api/cultivo/photo') maxBody = MAX_CULTIVO_MEDIA_BODY_BYTES;
    else if (url === '/api/cultivo') maxBody = MAX_CULTIVO_STATE_BODY_BYTES;
    if (bodyRaw.length > maxBody) {
      let tooLarge = 'body too large';
      if (url === '/api/admin/update-icons') tooLarge = 'imagem muito grande (máx. 8 MB)';
      else if (url === '/api/upload') tooLarge = 'imagem muito grande (máx. ~6 MB)';
      else if (url === '/api/cultivo/photo') tooLarge = 'mídia muito grande para envio';
      else if (url === '/api/cultivo') tooLarge = 'diário demasiado grande para gravar de uma vez';
      return jsonResponse(413, { error: tooLarge });
    }
  } catch (e) {
    return jsonResponse(400, { error: 'invalid body' });
  }

  const googleAuth = await handleGoogleAuth(req, { store });
  ...
  const routeModules = [
    ...
    require('./api-routes/series.js'),
    require('./api-routes/inspecoes-from-channel.js'),
    require('./api-routes/admin-tables.js'),
    require('./api-routes/cultivo-submissions.js'),
    require('./api-routes/community.js'),
    require('./api-routes/user-images.js'),
    require('./api-routes/radio.js'),
    require('./api-routes/etymology.js'),
    require('./api-routes/users-admin.js'),
    require('./api-routes/icons.js'),
    require('./api-routes/gtarp-streamers.js')
  ];
  ...
  if (url === '/api/auth/local/register' && method === 'POST') {
    try {
      ...
      if (!result.ok) return jsonResponse(result.status || 400, { error: result.error || 'Não foi possível criar conta.' });
  ...
  if (url === '/api/auth/local/login' && method === 'POST') {
    try {
      const payload = JSON.parse(bodyRaw || '{}');
      const result = await loginLocalUser(store, payload, authMeta);
      if (!result.ok) return jsonResponse(result.status || 401, { error: result.error || 'E-mail ou senha inválidos.' });
      const token = await createUserSession(store, result.user.id);
      return jsonResponse(200, { ok: true, user: publicUserView(result.user) }, {
        'Set-Cookie': setUserSessionCookie(headers, token)
      });
    } catch (e) {
      return jsonResponse(400, { error: 'Pedido inválido.' });
  ...
  if (url === '/api/auth/local/request-reset' && method === 'POST') {
    try {
      const payload = JSON.parse(bodyRaw || '{}');
  ...
  if (url === '/api/auth/local/reset' && method === 'POST') {
    try {
      const payload = JSON.parse(bodyRaw || '{}');
      const result = await resetLocalPassword(store, payload);
      if (!result.ok) return jsonResponse(result.status || 400, { error: result.error || 'Não foi possível atualizar a senha.' });
      const token = await createUserSession(store, result.user.id);
      return jsonResponse(200, { ok: true, user: publicUserView(result.user) }, {
        'Set-Cookie': setUserSessionCookie(headers, token)
      });
  ...
  if (url === '/api/user/profile' && method === 'PUT') {
    ...
    try {
      ...
      if (e && e.code === 'invalid_age') {
        return jsonResponse(400, { error: 'Informe uma idade válida (18 a 120).' });
      }
      if (e && e.code === 'invalid_username') {
        return jsonResponse(400, { error: 'Nome de utilizador inválido (use 3-32 caracteres: letras, números, . _ -).' });
      }
      if (e && e.code === 'invalid_whatsapp') {
        return jsonResponse(400, { error: 'Informe um WhatsApp válido com DDD (10 a 15 dígitos), ou deixe em branco.' });
      }
      return jsonResponse(400, { error: 'Pedido inválido.' });
    }
  }
  ...
  if (url === '/api/cultivo' && method === 'PUT') {
    const session = await getUserSession(store, cookie);
    if (!session) return jsonResponse(401, { error: 'authentication required' });
    const writeKey = getClientKey(req, headers) + ':' + session.userId;
    if (cultivoWriteLimiter.isLimited(writeKey)) {
      return jsonResponse(429, { error: 'Muitas gravações. Aguarde alguns minutos.' });
    }
    cultivoWriteLimiter.record(writeKey);
    try {
      const payload = JSON.parse(bodyRaw || '{}');
      ...
      return jsonResponse(400, { error: 'Não foi possível guardar este registo. Tente novamente.' });
    }
  }

  if (url === '/api/infra-plan' && method === 'GET') {
    const session = await getUserSession(store, cookie);
    if (!session) return jsonResponse(401, { error: 'authentication required' });
    const plan = await getInfraPlanForUser(store, session.userId);
    return jsonResponse(200, { ok: true, infraPlan: publicInfraPlanView(plan) });
  }

  if (url === '/api/infra-plan' && method === 'PUT') {
    const session = await getUserSession(store, cookie);
    if (!session) return jsonResponse(401, { error: 'authentication required' });
    const writeKey = getClientKey(req, headers) + ':' + session.userId;
    if (infraPlanWriteLimiter.isLimited(writeKey)) {
      return jsonResponse(429, { error: 'Muitas gravações. Aguarde alguns minutos.' });
    }
    infraPlanWriteLimiter.record(writeKey);
    try {
      const payload = JSON.parse(bodyRaw || '{}');
      const plan = await updateInfraPlanForUser(store, session.userId, payload);
      if (!plan) return jsonResponse(404, { error: 'utilizador não encontrado' });
      return jsonResponse(200, { ok: true, infraPlan: publicInfraPlanView(plan) });
    } catch (e) {
      return jsonResponse(400, { error: 'Pedido inválido.' });
    }
  ...
  if (url === '/api/cultivo/photo' && method === 'POST') {
    const session = await getUserSession(store, cookie);
    if (!session) return jsonResponse(401, { error: 'Sessão expirada. Entre de novo e tente enviar a foto.' });
    const photoKey = getClientKey(req, headers) + ':' + session.userId;
    if (cultivoPhotoLimiter.isLimited(photoKey)) {
      return jsonResponse(429, { error: 'Limite de envio de mídia atingido. Tente mais tarde.' });
    }
    try {
      const payload = JSON.parse(bodyRaw || '{}');
      let data = String(payload.data || '').replace(/\s+/g, '');
      if (!data) return jsonResponse(400, { error: 'mídia em falta' });
      const m = data.match(/^data:(image\/(?:png|jpeg|jpg|pjpeg|webp|gif)|video\/(?:mp4|webm|quicktime))(?:;charset=[^;]+)?;base64,(.+)$/i);
      if (!m) return jsonResponse(400, { error: 'formato de mídia inválido (use foto JPEG/PNG ou vídeo mp4/webm/mov)' });
      const mime = m[1].toLowerCase();
      const b64 = String(m[2] || '').replace(/[^A-Za-z0-9+/=]/g, '');
      const buf = Buffer.from(b64, 'base64');
      const isVideo = mime.startsWith('video/');
      // Fotos: 3.5 MB (base64 ~4.7 MB) para caber no limite típico de Functions (~6 MB).
      const maxBytes = isVideo ? 25 * 1024 * 1024 : Math.floor(3.5 * 1024 * 1024);
      if (!buf.length) {
        return jsonResponse(400, { error: 'foto vazia ou corrompida' });
      }
      if (buf.length > maxBytes) {
        return jsonResponse(413, {
          error: isVideo
            ? 'vídeo muito grande (máx. 25 MB)'
            : 'imagem muito grande (máx. 3,5 MB). Reduza a foto e tente de novo.'
        });
      }
      const extMap = {
        'image/png': '.png',
      ...
      const userId = String(session.userId || 'user').replace(/[^a-zA-Z0-9]/g, '').slice(0, 16) || 'user';
      const prefix = isVideo ? 'cultivo-video-' : 'cultivo-';
      const uniqueName = userImagesPrefix(session.userId) + prefix + userId + '-' + Date.now() + ext;
      let publicUrl;
      try {
        publicUrl = await store.saveUpload(uniqueName, buf);
      } catch (storeErr) {
        console.error('[cultivo/photo] saveUpload', storeErr && storeErr.stack ? storeErr.stack : storeErr);
        return jsonResponse(500, { error: 'Erro no servidor ao guardar a foto. Tente de novo.' });
      }
      cultivoPhotoLimiter.record(photoKey);
      return jsonResponse(201, { ok: true, url: publicUrl });
    } catch (e) {
      console.error('[cultivo/photo]', e && e.message ? e.message : e);
      return jsonResponse(400, { error: 'upload inválido (formato ou dados da foto)' });
    }
  }
  ...
  if (url === '/api/videos-hub' && method === 'GET') {
    try {
      const { buildVideosHub } = require('./videos-hub.js');
      const hubRoot = fsFallback || root || process.cwd();
      let youtubeFeed = null;
      if (store.getYoutubeFeed) {
        try {
          youtubeFeed = await store.getYoutubeFeed();
        } catch (e) {
          youtubeFeed = null;
        }
      }
      const hub = buildVideosHub(hubRoot, { youtubeFeed: youtubeFeed || undefined });
      return jsonResponse(200, hub);
    } catch (e) {
      try {
        const hubPath = path.join(fsFallback || root || process.cwd(), 'content', 'videos-hub.json');
        const hub = JSON.parse(fs.readFileSync(hubPath, 'utf8'));
        return jsonResponse(200, hub);
      } catch (e2) {
        return jsonResponse(404, { error: 'videos hub not found' });
      }
    }
  }
  ...
  if (url === '/api/auth/registration-status' && method === 'GET') {
    const {
      areNewRegistrationsAllowed,
      REGISTRATION_CLOSED_MESSAGE
    } = require('./registration-gate.js');
