...
async function handleApiRequest(req, options) {
  ...
  if (url === '/api/cultivo/photo' && method === 'POST') {
    ...
    try {
      ...
      const m = data.match(/^data:((?:image\/(?:png|jpeg|jpg|pjpeg|webp|gif))|(?:video\/(?:mp4|webm|quicktime)));base64,(.+)$/i);
      if (!m) return jsonResponse(400, { error: 'formato de mídia inválido (use foto ou vídeo mp4/webm/mov)' });
      ...
      const uniqueName = prefix + userId + '-' + Date.now() + ext;
      let publicUrl;
      try {
