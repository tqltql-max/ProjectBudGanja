...
async function handleApiRequest(req, options) {
  ...
  const routeModules = [
    ...
    require('./api-routes/email-admin.js')
  ];
  ...
  if (url === '/api/auth/google/callback' && method === 'GET') {
    ...
    const { user, isNew } = await upsertGoogleUser(store, googleUser);
    if (isNew) {
      sendWelcomeEmail(store, user).catch((e) => {
  ...
  if (url === '/api/cultivo/photo' && method === 'POST') {
    ...
    if (cultivoPhotoLimiter.isLimited(photoKey)) {
      return jsonResponse(429, { error: 'Limite de envio de fotos atingido. Tente mais tarde.' });
    }
