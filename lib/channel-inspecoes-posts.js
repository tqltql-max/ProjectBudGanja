...
function loadMovrecamCatalog() {
  return readJsonSafe(path.join(ROOT, 'content', 'channels', 'movrecam.json'), { videos: [] });
}

function loadCanabinallCatalog() {
  return readJsonSafe(path.join(ROOT, 'content', 'channels', 'canabinall.json'), { videos: [] });
}

function loadInspetorBudGanjaCatalog() {
  return readJsonSafe(path.join(ROOT, 'content', 'youtube-feed.json'), { videos: [] });
}
...
As **${xivWindow.length} aulas** catalogadas desta edição estão embutidas abaixo, em ordem cronológica, para os alunos reverem aqui no site. Também podes abrir o [hub de vídeos filtrado (MovReCam · XIV)](/videos/?channel=movrecam&series=xiv) — o arquivo oficial continua no [canal MovReCam](${ch.channelUrl}).
...
${xivWatchBlocks}`
  ...
  let professorsSection = '';
  try {
    const { categorizeMovrecamProfessors } = require('./movrecam-professor-series.js');
    const doc = categorizeMovrecamProfessors();
    const top = (doc.summary || []).slice(0, 30);
    if (top.length) {
      const rows = top
        .map((p) => {
          const filter = '/videos/?channel=movrecam&series=' + p.id;
          const fiche = p.inspectionUrl ? ' · [ficha](' + p.inspectionUrl + ')' : '';
          return (
            '| [' +
            p.label +
            '](' +
        ...
        .join('\n');
      professorsSection =
        `## Professores e palestrantes (catálogo por nome)

Os títulos do MovReCam nomeiam dezenas de professores. No hub de vídeos, escolhe **MovReCam** e filtra pela série do professor (\`/videos/?channel=movrecam&series=<id>\`).

| Professor / palestrante | Vídeos | Links |
|-------------------------|--------|-------|
${rows}

_Total: **${doc.professorCount}** nomes · **${doc.taggedVideoCount}**/${doc.catalogVideoCount} vídeos com nome no título._
`;
    }
  } catch (e) {
    professorsSection = '';
...
7. **Catálogo por professor** — nomes nos títulos viram filtros de série no hub (Eliana, Gabrielle, Sidarta, Renata Monteiro, etc.).
...
| Assistir aulas no site | [Hub de vídeos · MovReCam XIV](/videos/?channel=movrecam&series=xiv) |
| Filtrar por professor | [Hub · MovReCam](/videos/?channel=movrecam) · [Eliana](/videos/?channel=movrecam&series=eliana) · [Gabrielle](/videos/?channel=movrecam&series=gabrielle) · [Sidarta](/videos/?channel=movrecam&series=sidarta) |
...
**Aprovado como referência de conteúdo e arquivo** — canal essencial para acompanhar e rever as aulas UNIFESP, com acervo histórico de advocacy em português. A secção **Assistir aulas da XIV edição** reúne os embeds para estudo in-page; o [hub de vídeos](/videos/?channel=movrecam&series=xiv) concentra o mesmo filtro. Cruzar módulos de cultivo com medições no [laboratório BudGanja](/cultivo/) e validar regras de certificado no [SIEX](${ch.unifespCourseUrl}).

[▶ Vídeos MovReCam · XIV](/videos/?channel=movrecam&series=xiv) · [Canal MovReCam](${ch.channelUrl}) · [Inspeção do curso UNIFESP](/posts/post-inspecao-curso-unifesp-cannabis-medicinal.html) · [Todas as inspeções](/biblioteca/inspecoes/)`;
}
...
|------------------------------------|------------------|
| Assistir no site | [Hub de vídeos · CANABinALL](/videos/?channel=canabinall) · [Conceitos básicos](/videos/?channel=canabinall&series=conceitos) · [Plantas Sagradas](/videos/?channel=canabinall&series=plantas-sagradas) |
| Aulas longas UNIFESP (mesma coordenadora RTC) | [Canal MovReCam](/posts/post-inspecao-canal-movrecam.html) · [Curso UNIFESP](/posts/post-inspecao-curso-unifesp-cannabis-medicinal.html) · [Vídeos MovReCam XIV](/videos/?channel=movrecam&series=xiv) |
...
- **Dedicatória do projeto:** [Prof. Emérito Elisaldo Carlini](/posts/post-inspecao-elisaldo-carlini.html)
- **Inspeção redigida por:** Inspetor BudGanja (laboratório digital independente)
...
[▶ Conceitos básicos #1](/videos/?channel=canabinall&series=conceitos#${CANABINALL_HIGHLIGHT.id}) · [◆ Plantas Sagradas](/videos/?channel=canabinall&series=plantas-sagradas#${CANABINALL_SIGNATURE.id}) · [Hub CANABinALL](/videos/?channel=canabinall) · [Canal YouTube](${ch.channelUrl}) · [MovReCam](/posts/post-inspecao-canal-movrecam.html) · [Curso UNIFESP](/posts/post-inspecao-curso-unifesp-cannabis-medicinal.html) · [Todas as inspeções](/biblioteca/inspecoes/)`;
}
