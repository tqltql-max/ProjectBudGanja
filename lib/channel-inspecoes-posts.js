...
|---------------|--------------------------------|
| Início do ciclo | [Diário de Cultivo](/cultivo/) · [Diário de Cultivo](/cultivo/) |
...
const MOVRECAM_HIGHLIGHT = {
  id: 'dV-6qKW1HXg',
  startSeconds: 3894,
  title: '11ª Aula - Psicodélicos - Profª Dra. Eliana Rodrigues'
};
...
function buildMovrecamBody(ch) {
  ...
  const xivWindow = videos.filter((v) => {
  ...
  const highlight = videos.find((v) => v.id === MOVRECAM_HIGHLIGHT.id) || {
    id: MOVRECAM_HIGHLIGHT.id,
    title: MOVRECAM_HIGHLIGHT.title,
    url: 'https://www.youtube.com/watch?v=' + MOVRECAM_HIGHLIGHT.id,
    published: '2026-07-23T22:39:36+00:00',
    views: '4,4 mil visualizações'
  };
  const highlightStamp = formatTimestamp(MOVRECAM_HIGHLIGHT.startSeconds);
  const highlightUrl =
    'https://www.youtube.com/watch?v=' +
    MOVRECAM_HIGHLIGHT.id +
    '&t=' +
    MOVRECAM_HIGHLIGHT.startSeconds +
    's';
  ...
  const catalogRows = byDate
    .map((v, i) => {
      ...
      const mark = v.id === MOVRECAM_HIGHLIGHT.id ? ' ★' : '';
      return '| ' + (i + 1) + ' | ' + videoMd(v) + mark + ' | ' + date + ' | ' + views + ' |';
    })
    .join('\n');

  const refId = MOVRECAM_HIGHLIGHT.id;
  const refEmbed = refId + '?start=' + MOVRECAM_HIGHLIGHT.startSeconds;
...
**[${highlight.title}](${highlightUrl})** — Profa. Dra. Eliana Rodrigues (coordenação RTC do curso UNIFESP).

| Campo | Valor |
|-------|-------|
| Vídeo | [${MOVRECAM_HIGHLIGHT.id}](https://www.youtube.com/watch?v=${MOVRECAM_HIGHLIGHT.id}) |
| Publicação | ${highlight.published ? highlight.published.slice(0, 10) : '2026-07-23'} |
| Views (snapshot) | ${highlight.views || '—'} |
| Momento destacado | **${highlightStamp}** (t=${MOVRECAM_HIGHLIGHT.startSeconds}s) |
| Link directo | [Abrir no minuto ${highlightStamp}](${highlightUrl}) |

Ponto de entrada recomendado nesta passagem: a **11ª aula da XIV edição** sobre psicodélicos — útil para cruzar saúde mental, redução de danos e debate científico com a grade clínica do curso. O embed abaixo abre já no instante marcado.
...
- **Método:** (1) inventário via página pública \`/videos\` (${ch.videoCount} uploads); (2) classificação temática por palavras-chave; (3) destaque editorial de uma aula recente da XIV edição; (4) cruzamento com a grade SIEX e canais oficiais.
...
3. **Bloco de saúde mental / psicodélicos em expansão** — a aula destacada (Eliana Rodrigues) soma-se a episódios sobre ansiedade, depressão e endocanabinoidoma.
...
6. **Presença para certificado** — cada aula ao vivo disponibiliza formulário Google (e-mail igual ao da inscrição SIEX); o arquivo YouTube serve consulta, não substitui o registo de frequência.
...
**Aula em destaque** — [${highlight.title}](${highlightUrl}) · início em ${highlightStamp}:

@youtube ${refEmbed}

Abertura da XIV edição (orientações de inscrição, presença e certificado):
...
| Plantio, clonagem, cultivo orgânico | [Propagação e clonagem](/equipamentos/clonadora-6-estacas.html) · [Diário de cultivo](/cultivo/) |
...
- **Aula destacada:** © MovReCam / Profa. Dra. Eliana Rodrigues — [Psicodélicos](${highlightUrl})
...
**Aprovado como referência de conteúdo e arquivo** — canal essencial para acompanhar e rever as aulas UNIFESP, com acervo histórico de advocacy em português. Nesta passagem, a **11ª aula (Psicodélicos)** é o ponto de entrada recomendado. Cruzar módulos de cultivo com medições no [laboratório BudGanja](/cultivo/) e validar regras de certificado no [SIEX](${ch.unifespCourseUrl}).

[▶ Abrir aula destacada](${highlightUrl}) · [Canal MovReCam](${ch.channelUrl}) · [Inspeção do curso UNIFESP](/posts/post-inspecao-curso-unifesp-cannabis-medicinal.html) · [Todas as inspeções](/biblioteca/inspecoes/)`;
}
...
function buildMovrecamInspectionPost() {
  ...
  return {
    ...
    excerpt:
      ...
      ch.videoCount +
      ' vídeos catalogados; destaque à 11ª aula (Psicodélicos, Profa. Dra. Eliana Rodrigues) da XIV edição UNIFESP.',
...
| Propagação e clonagem (quando o tema surgir em congressos) | [Propagação](/equipamentos/clonadora-6-estacas.html) · [Diário de cultivo](/cultivo/) |
