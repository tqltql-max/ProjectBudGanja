...
| **Par lab** | As duas juntas | Sem verbo, a peça é muda; sem peça, o verbo fica abstracto |

**Tese:** [ligar × desligar](${ligarDesligar}) é a ficha-irmã dos **verbos**; aqui fica o **objecto** e as derivações de *interrompere*.
...
**Aprovado** — **interruptor** fichado com derivações (*interromper*, *interrupção*…), [ligar/desligar](${ligarDesligar}) × corte de fluxo e [Faça o melhor!](${mantra}).

[▶ Palavras](${hub}) · [▶ Ligar × Desligar](${ligarDesligar}) · [▶ Sinal](${sinal}) · [▶ Gesto](${gesto}) · [▶ Eletrizante](${eletrizante}) · [▶ Faça o melhor!](${mantra}) · [▶ Vida](${vida})
`;

  const contentEn = `## Scope

Editorial inspection of Portuguese **interruptor** (switch / interrupter) — the device that [turns on/off](${ligarDesligar}) and, by lived extension, the point that **cuts or resumes a flow**. Covers **object** (Lat. *interrumpere*), **derivatives** (*interromper*, *interrupção*…), links to [ligar × desligar](${ligarDesligar}), [sinal](${sinal}), [gesto](${gesto}), [eletrizante](${eletrizante}), [pular](${pular}), and [Do your best!](${mantra}).
...
**Approved** — object · derivatives · [ligar × desligar](${ligarDesligar}) × cut · [Do your best!](${mantra}).

[▶ Words](${hub}) · [▶ Ligar × Desligar](${ligarDesligar}) · [▶ Sinal](${sinal}) · [▶ Gesto](${gesto}) · [▶ Do your best!](${mantra})
`;

  const contentEs = `## Alcance

Inspección de **interruptor** — el dispositivo que [enciende/apaga](${ligarDesligar}) y, por extensión vivida, el punto que **corta o retoma un flujo**. Cubre **objeto** (lat. *interrumpere*), **derivaciones** (*interromper*, *interrupção*…), vínculos con [ligar × desligar](${ligarDesligar}), [sinal](${sinal}), [gesto](${gesto}), [eletrizante](${eletrizante}), [pular](${pular}) y [¡Haz lo mejor!](${mantra}).
...
[▶ Palabras](${hub}) · [▶ Ligar × Desligar](${ligarDesligar}) · [▶ Sinal](${sinal}) · [▶ Gesto](${gesto}) · [▶ ¡Haz lo mejor!](${mantra})
`;
...
function buildInterruptorPost(seriesOrder) {
  ...
  return makePalavra({
    ...
    excerpt:
      'Palavras: «interruptor» (lat. *interrumpere*) — peça do circuito; par lab com ligar × desligar; derivações interromper/interrupção; Faça o melhor!',
    excerptEn:
      'Words: “interruptor” (Lat. *interrumpere*) — circuit device; lab pair with ligar × desligar; derivatives interromper/interrupção; Do your best!',
    excerptEs:
      'Palabras: «interruptor» (lat. *interrumpere*) — pieza del circuito; par lab con ligar × desligar; derivaciones interromper/interrupção; ¡Haz lo mejor!',
    slug: 'inspecao-palavra-interruptor',
    date: '2026-08-03T15:20:00.000Z',
    seriesOrder: order,
    seriesLabel: 'Interruptor · ligar × desligar',
