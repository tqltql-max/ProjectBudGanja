...
const { readPlantas } = require('./plantas-service.js');
const {
  getUserById,
  publicUserView,
...
function getPlantIndex() {
  if (plantIndexCache) return plantIndexCache;
  const catalog = readPlantas();
  const plants = Array.isArray(catalog.plants) ? catalog.plants : [];
  plantIndexCache = plants
    .map((p) => {
      const aliases = [
        p.nomePopular,
        p.nomeCientifico,
        p.slug,
        p.i18n && p.i18n.en && p.i18n.en.nomePopular,
        p.i18n && p.i18n.es && p.i18n.es.nomePopular
      ]
        .map(normalizePlantMatchText)
        .filter(Boolean);
      // Prefer longer aliases first so "capim limao" beats "limao" if both exist.
    ...
    .filter((p) => p.slug);
  return plantIndexCache;
}

function plantLinksFromSlug(slug, nomePopular) {
  if (!slug) return null;
  return {
    plantSlug: slug,
    plantName: nomePopular || slug,
    plantFichaUrl: '/plantas/' + slug + '/',
    plantInspectionUrl: '/posts/post-inspecao-planta-' + slug + '.html'
  };
}

function textHasAlias(blob, alias) {
  if (!blob || !alias) return false;
...
function matchPlantFromText() {
  const parts = Array.prototype.slice.call(arguments);
  const blob = normalizePlantMatchText(parts.filter(Boolean).join(' '));
  if (!blob) return null;
  const index = getPlantIndex();
  for (let i = 0; i < index.length; i++) {
    const plant = index[i];
    for (let j = 0; j < plant.aliases.length; j++) {
      if (textHasAlias(blob, plant.aliases[j])) {
        return plantLinksFromSlug(plant.slug, plant.nomePopular);
      }
    }
  }
  return null;
...
function resolvePlantLinksForPost(post, comments) {
  if (!post) return null;
  const fromGrow = matchPlantFromText(post.growSpecies, post.growName);
  if (fromGrow) return fromGrow;

  const commentBodies = (comments || [])
    .map((c) => (c && c.body) || '')
    .join(' ');
  return matchPlantFromText(post.caption, commentBodies);
}
...
function publicPostView(post, plantLinks) {
  if (!post) return null;
  ...
  if (plantLinks && plantLinks.plantSlug) {
    view.plantSlug = plantLinks.plantSlug;
    view.plantName = plantLinks.plantName || plantLinks.plantSlug;
    view.plantFichaUrl = plantLinks.plantFichaUrl;
    view.plantInspectionUrl = plantLinks.plantInspectionUrl;
  }
  return view;
}
...
function photoOwnedByUser(photoUrl, userId) {
  ...
  if (uid && url.indexOf('/cultivo-' + uid + '-') !== -1) return true;
  if (uid && url.indexOf('/cultivo-video-' + uid + '-') !== -1) return true;
  if (uid && url.startsWith('/uploads/cultivo-' + uid + '-')) return true;
  const folderId = String(userId || '').replace(/[^a-zA-Z0-9_-]/g, '').slice(0, 64);
  return !!folderId && url.startsWith('/uploads/users/' + folderId + '/');
}

function peekPlantIdRate(userId) {
  const now = Date.now();
  const key = String(userId);
  const bucket = plantIdRateMap.get(key);
  if (!bucket || now - bucket.start > PLANT_ID_RATE_WINDOW_MS) return true;
  return bucket.count < PLANT_ID_RATE_MAX;
}

function recordPlantIdRate(userId) {
  const now = Date.now();
  const key = String(userId);
...
async function getCommunityFeed(store, options) {
  ...
  for (const post of result.items || []) {
    let comments = [];
    try {
      comments = await store.listCommunityComments(post.id);
    } catch (e) {
      comments = [];
    }
    const plantLinks = resolvePlantLinksForPost(post, comments);
    const view = publicPostView(post, plantLinks);
    if (!view) continue;
    view.comments = (comments || []).map(publicCommentView).filter(Boolean);
    items.push(view);
  }
  return {
    items,
