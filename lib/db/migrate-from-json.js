...
function userToRow(user) {
  ...
  const candidateUsername = String(user.username || profile.username || '').trim().toLowerCase();
  return {
    id: String(user.id),
    ...
    birth_date: String(user.birthDate || profile.birthDate || ''),
    picture: String(user.picture || ''),
    provider: String(user.provider || 'google'),
    registration_ip: String(user.registrationIp || ''),
    last_login_at: user.lastLoginAt || null,
    last_login_ip: String(user.lastLoginIp || ''),
    account_status: String(user.accountStatus || 'pending_profile'),
    onboarding_stage: String(user.onboardingStage || 'initial'),
    activity_log_json: JSON.stringify(Array.isArray(user.activityLog) ? user.activityLog : []),
...
async function importUsersFromJson(db, root) {
  ...
  for (const user of entries) {
    ...
    await db.execute({
      sql: `INSERT INTO users (id, email, email_verified_at, name, username, birth_date, picture, provider, registration_ip, last_login_at, last_login_ip, account_status, onboarding_stage, activity_log_json, profile_json, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
