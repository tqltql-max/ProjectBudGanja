...
function loadCreateClient() {
  if (process.env.CF_PAGES) {
    try {
      return require('@libsql/client/web').createClient;
    } catch (e) {
      try {
...
function resolveDatabaseUrl(root) {
  ...
  try {
    ...
    throw new Error('Base de dados local indisponível. Defina TURSO_DATABASE_URL na Cloudflare Pages.');
  }
...
async function ensureSchema(db) {
  ...
  try {
    ...
    current = row.rows[0] && row.rows[0].v != null ? Number(row.rows[0].v) : 0;
  } catch (e) {
    if (!loadSchemaSql()) return;
    throw e;
  }
  ...
  const incremental = [
    ...
    },
    {
      version: 19,
      statements: [
        `INSERT OR IGNORE INTO post_series (id, label, category, sort_order, created_at) VALUES ('formacao-academica', 'Extens\u00e3o acad\u00e9mica', 'inspecao', 6, '${new Date().toISOString()}')`
      ]
    },
    {
      version: 20,
    ...
    },
    {
      version: 22,
      statements: [
        "ALTER TABLE site_settings ADD COLUMN spotify_podcast_url TEXT NOT NULL DEFAULT ''",
        "ALTER TABLE site_settings ADD COLUMN spotify_podcast_label TEXT NOT NULL DEFAULT ''",
        "UPDATE site_settings SET og_image = '/imagens/background-hero.png', spotify_podcast_url = 'https://open.spotify.com/show/033yuLDWnN84xOcfHyJ1FZ', spotify_podcast_label = 'Podcast Inspetor BudGanja' WHERE id = 'default'"
      ]
    },
    {
      version: 23,
      statements: [
        "DELETE FROM posts WHERE slug = 'inspecao-canal-jardimhg' OR series = 'canal-jardimhg'",
        "DELETE FROM post_series WHERE id = 'canal-jardimhg'"
      ]
    },
    {
      version: 24,
      statements: [
        "DELETE FROM posts WHERE slug = 'inspecao-insumo-biobizz'"
      ]
    },
    {
      version: 25,
      statements: [
        "DELETE FROM posts WHERE slug = 'inspecao-loja-floraurbana'"
      ]
    },
    {
      version: 26,
      statements: [
        "ALTER TABLE posts ADD COLUMN excerpt_en TEXT NOT NULL DEFAULT ''",
        "ALTER TABLE posts ADD COLUMN excerpt_es TEXT NOT NULL DEFAULT ''"
      ]
    },
    {
      version: 27,
      statements: [
        `INSERT OR IGNORE INTO post_series (id, label, category, sort_order, created_at) VALUES ('artigos-cientificos', 'Artigos cient\u00edficos', 'inspecao', 7, '${new Date().toISOString()}')`
      ]
    },
    {
      version: 28,
      statements: [
        "ALTER TABLE youtube_feed_videos ADD COLUMN thumb TEXT NOT NULL DEFAULT ''",
        "ALTER TABLE youtube_feed_videos ADD COLUMN title_en TEXT NOT NULL DEFAULT ''",
        "ALTER TABLE youtube_feed_videos ADD COLUMN title_es TEXT NOT NULL DEFAULT ''",
        "ALTER TABLE youtube_feed_videos ADD COLUMN summary_en TEXT NOT NULL DEFAULT ''",
        "ALTER TABLE youtube_feed_videos ADD COLUMN summary_es TEXT NOT NULL DEFAULT ''"
      ]
    },
    {
      version: 29,
      statements: [
        `INSERT OR IGNORE INTO post_series (id, label, category, sort_order, created_at) VALUES ('legado-pessoas', 'Legado', 'inspecao', 1, '${new Date().toISOString()}')`
      ]
    },
    {
      version: 30,
      statements: [
        "ALTER TABLE site_settings ADD COLUMN ga_measurement_id TEXT NOT NULL DEFAULT 'G-Q47PEYEXX6'",
        "UPDATE site_settings SET ga_measurement_id = 'G-Q47PEYEXX6' WHERE id = 'default' AND (ga_measurement_id IS NULL OR ga_measurement_id = '')"
      ]
    },
    {
      version: 31,
      statements: [
        `CREATE TABLE IF NOT EXISTS gtarp_streamer_applications (
          id TEXT PRIMARY KEY,
          status TEXT NOT NULL DEFAULT 'pending',
          ...
          kick_handle TEXT NOT NULL DEFAULT '',
        ...
        'CREATE INDEX IF NOT EXISTS idx_gtarp_apps_status ON gtarp_streamer_applications(status)',
        'CREATE INDEX IF NOT EXISTS idx_gtarp_apps_submitted ON gtarp_streamer_applications(submitted_at)'
      ]
    }
  ];
