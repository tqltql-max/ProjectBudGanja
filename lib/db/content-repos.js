...
async function saveSite(db, site) {
  ...
  await db.execute({
    sql: `INSERT INTO site_settings (id, site_name, site_tagline, footer_text, privacy_updated, og_image,
          ...
          spotify_podcast_url, spotify_podcast_label, ga_measurement_id, updated_at)
          VALUES ('default', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    args: [
      ...
      String(site.gaMeasurementId || 'G-Q47PEYEXX6'),
      nowIso()
    ]
  });
...
async function loadSite(db) {
  ...
  return {
    ...
    gaMeasurementId: s.ga_measurement_id || 'G-Q47PEYEXX6',
    nav: nav,
...
async function contentTableCounts(db) {
  const tables = [
    ...
    'sorteio_alert_subscribers', 'youtube_feed', 'youtube_feed_videos', 'users'
  ];
  const out = {};
