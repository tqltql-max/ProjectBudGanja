...
function rowToPost(row) {
  return {
    ...
    series: row.series || '',
    seriesOrder: row.series_order != null ? Number(row.series_order) : null,
...
async function loadPosts(db) {
  const r = await db.execute(
    `SELECT slug, title, excerpt, excerpt_en, excerpt_es, content_raw, format, filename, url, date,
            published, cover_image, category, series, series_order, series_label
...
async function savePosts(db, posts) {
  ...
  for (const p of posts || []) {
    ...
    await db.execute({
      sql: `INSERT INTO posts (slug, title, excerpt, excerpt_en, excerpt_es, content_raw, format, filename, url, date,
                              published, cover_image, category, series, series_order, series_label, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      args: [
        String(p.slug),
        String(p.title || ''),
        ...
        String(p.series || ''),
        p.seriesOrder != null ? Number(p.seriesOrder) : null,
