...
async function postsSupportLocaleExcerpts(db) {
  try {
    await db.execute('SELECT excerpt_en, excerpt_es FROM posts LIMIT 1');
    return true;
  } catch (e) {
    return false;
  }
...
async function loadPosts(db) {
  // series/series_label (v16) e excerpt_en/es (v26) — fallback se a migração ainda não correu
  const withLocale = await postsSupportLocaleExcerpts(db);
  const r = await db.execute(
    withLocale
      ? `SELECT slug, title, excerpt,
            COALESCE(excerpt_en,'') AS excerpt_en, COALESCE(excerpt_es,'') AS excerpt_es,
            content_raw, format, filename, url, date, published, cover_image, category,
...
async function savePosts(db, posts) {
  ...
  for (const p of posts || []) {
    ...
    if (withLocale) {
      await db.execute({
        sql: `INSERT INTO posts (slug, title, excerpt, excerpt_en, excerpt_es, content_raw, format, filename, url, date, published, cover_image, category, series, series_label, updated_at)
              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        args: [
          String(p.slug),
          String(p.title || ''),
          String(p.excerpt || ''),
          String(p.excerptEn || p.excerpt_en || ''),
          String(p.excerptEs || p.excerpt_es || ''),
...
async function saveSite(db, site) {
  ...
  await db.execute({
    sql: `INSERT INTO site_settings (id, site_name, site_tagline, footer_text, privacy_updated, og_image,
          contact_email, youtube_channel_url, youtube_channel_label, youtube_jardim_url, youtube_jardim_label,
          spotify_podcast_url, spotify_podcast_label, updated_at)
          VALUES ('default', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
          ON CONFLICT(id) DO UPDATE SET
            ...
            spotify_podcast_url = excluded.spotify_podcast_url,
            spotify_podcast_label = excluded.spotify_podcast_label,
    ...
    args: [
      ...
      String(site.spotifyPodcastUrl || ''),
      String(site.spotifyPodcastLabel || ''),
      nowIso()
    ]
  });
...
async function loadSite(db) {
  ...
  return {
    ...
    spotifyPodcastUrl: s.spotify_podcast_url || '',
    spotifyPodcastLabel: s.spotify_podcast_label || '',
    nav: nav,
...
async function saveSorteios(db, entries) {
  ...
  for (const e of entries || []) {
    const normalized = normalizeSorteioEntryInput(e, nowIso());
    ...
    await db.execute({
      ...
      args: [
        ...
        normalized.email,
        normalized.cpf,
        normalized.cpfFormatado,
        normalized.telefone,
        normalized.cidade,
        normalized.estado,
        normalized.instagram,
        normalized.premioId,
        normalized.premioLabel,
...
async function loadSorteios(db) {
  ...
  return r.rows.map((row) => normalizeSorteioEntryOutput({
    id: row.id,
    user_id: row.user_id,
    nome: row.nome,
    email: row.email,
...
async function saveLojaOrders(db, orders) {
  ...
  for (const o of list) {
    const normalized = normalizeLojaOrderInput(o, nowIso());
    if (!normalized.id) continue;
    await db.execute({
      ...
      args: [
        ...
        normalized.packageLabel,
        normalized.packagePriceNote,
        normalized.nome,
        normalized.email,
        normalized.telefone,
        normalized.cidade,
...
async function loadLojaOrders(db) {
  ...
  return r.rows.map((row) => normalizeLojaOrderOutput({
    id: row.id,
    product_id: row.product_id,
    product_title: row.product_title,
    package_id: row.package_id,
    package_label: row.package_label,
    package_price_note: row.package_price_note,
    nome: row.nome,
    email: row.email,
    telefone: row.telefone,
    cidade: row.cidade,
...
async function saveYoutubeFeed(db, feed) {
  ...
  for (let i = 0; i < videos.length; i++) {
    ...
    await db.execute({
      sql: `INSERT INTO youtube_feed_videos (
              youtube_id, title, published, summary, url, thumb,
              title_en, title_es, summary_en, summary_es, sort_order
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      args: [
        id,
        String(v.title || ''),
  ...
  return {
    ...
    videos: videosRows.rows.map((v) => {
      ...
      const item = {
        ...
        url: v.url,
        thumb:
          (v.thumb && String(v.thumb).trim()) ||
          (id ? 'https://i.ytimg.com/vi/' + id + '/hqdefault.jpg' : '')
      };
      if (v.title_en) item.titleEn = v.title_en;
      if (v.title_es) item.titleEs = v.title_es;
      if (v.summary_en) item.summaryEn = v.summary_en;
      if (v.summary_es) item.summaryEs = v.summary_es;
      return item;
    })
  };
}
