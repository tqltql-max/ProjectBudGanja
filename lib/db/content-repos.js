...
async function loadPosts(db) {
  const r = await db.execute(
    'SELECT slug, title, excerpt, content_raw, format, filename, url, date, published, cover_image, category FROM posts ORDER BY date DESC'
  );
...
async function savePosts(db, posts) {
  ...
  for (const p of posts || []) {
    ...
    await db.execute({
      sql: `INSERT INTO posts (slug, title, excerpt, content_raw, format, filename, url, date, published, cover_image, category, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      args: [
        String(p.slug),
        String(p.title || ''),
