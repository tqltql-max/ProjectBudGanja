...
function mapUserRow(row) {
  ...
  return {
    ...
    accountStatus: row.account_status || 'pending_profile',
    onboardingStage: row.onboarding_stage || 'initial',
...
async function deleteUserById(db, userId) {
  ...
  if (!id) return false;
  const existing = await getUserByIdAdmin(db, id);
  if (!existing) return false;
  // Limpa referências soltas (sem FK) para não deixar IDs órfãos.
  try {
    await db.execute({
      sql: "UPDATE sorteio_entries SET user_id = '' WHERE user_id = ?",
      args: [id]
    });
  } catch (e) { /* coluna pode não existir em bases antigas */ }
  try {
    await db.execute({
      sql: "UPDATE loja_orders SET user_id = '' WHERE user_id = ?",
      args: [id]
    });
  } catch (e) { /* ignore */ }
  await db.execute({
