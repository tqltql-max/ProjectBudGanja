...
async function loadCultivoGrows(db, userId) {
  const growsResult = await db.execute({
    sql: `SELECT id, name, planted_at, phase, plant_count, species, created_at, updated_at
...
async function saveCultivoState(db, userId, state) {
  ...
  for (const grow of growLogs) {
    ...
    await db.execute({
      sql: `INSERT INTO cultivo_grows (id, user_id, name, planted_at, phase, plant_count, species, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      args: [
        grow.id,
        userId,
