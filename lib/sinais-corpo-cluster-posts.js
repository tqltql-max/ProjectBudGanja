...
const H = {
  ...
  cigarro: '/posts/post-inspecao-palavra-cigarro.html',
  cigarra: '/posts/post-inspecao-palavra-cigarra.html',
  orelhaCola: '/posts/post-inspecao-expressao-a-orelha-cola-o-que-a-boca-juntou.html',
  cola: '/posts/post-inspecao-palavra-cola-colar.html'
};
...
Inspeção editorial da palavra **[orelha](${H.orelha})** — o **pavilhão** que ouve e, no lab, o sítio onde a fala **cola**. Pedido de campo: *inspeção em Orelha* (depois de [cigarro](${H.cigarro}) × [cigarra](${H.cigarra})). Já havia leitura de **[sinal](${H.sinal}) de curiosidade** («pulga atrás da orelha»). Esta ficha junta os dois ofícios: a **pulga** (pergunta que não assenta) e a **cola** ([a orelha cola o que a boca juntou](${H.orelhaCola})).
...
| Elo ouvir | [Deus deu dois ouvidos](${H.ouvidos}) — **ouvido** (escuta) ≠ **orelha** (pavilhão / cola) |
| Elo cola | [a orelha cola…](${H.orelhaCola}) · [cola](${H.cola}) |
| Elo caso | [cigarro](${H.cigarro}) × [cigarra](${H.cigarra}) — uma letra; a orelha funde |
...
**H4:** **orelha** (pavilhão / figura) ≠ **ouvido** (órgão da escuta no ditado).  
**H5:** [a orelha cola o que a boca juntou](${H.orelhaCola}) — *cigarro* / *cigarra* é o caso: a boca disse as duas; a orelha cola **antes** do étimo cortar.

## 3. Caso de campo — cigarro × cigarra

Pedido irmão: *relação com inseto cigarra*. A [cigarra](${H.cigarra}) **canta** para a orelha; o [cigarro](${H.cigarro}) **não** é insecto. A cola é **som/grafia**, não biologia.

| Entra na orelha | O que é | O que a cola finge |
|-----------------|---------|-------------------|
| **[Cigarro](${H.cigarro})** | Rolo industrial (BR) | O mesmo que a cigarra |
| **[Cigarra](${H.cigarra})** | [Inseto](${H.inseto}) que canta | Um cigarro com asas |
| **Pulga atrás** | Figura de curiosidade | Insecto clínico |

**Correção:** nomear as **duas frases** (produto · insecto) — o ofício da [cola](${H.cola}).
...
| Cortar *cigarro* / *cigarra* em duas fichas | Deixar a orelha fundir produto e insecto |
...
| Anti-armadilha | Pulga ≠ prova · curiosidade ≠ acusação · cola ≠ étimo |
| Mapa | [sinais](${H.sinais}) · [orelha cola](${H.orelhaCola}) |

**Veredicto:** Valeu !!! **com a orelha como sinal de curiosidade e de cola** — a pulga pede inspeção; *cigarro* / *cigarra* pedem **duas salas**.

## Status

**Aprovado** — orelha fichada: *auricula* · pulga atrás · cola do lab · caso cigarro/cigarra; [Valeu !!!](${H.mantra}).

[▶ Sinais](${H.sinais}) · [▶ Orelha cola](${H.orelhaCola}) · [▶ Cigarro](${H.cigarro}) · [▶ Cigarra](${H.cigarra}) · [▶ Barriga](${H.barriga}) · [▶ Inseto](${H.inseto}) · [▶ Dois ouvidos](${H.ouvidos}) · [▶ Valeu !!!](${H.mantra})
`;
  const { contentEn, contentEs } = shortEnEs({
    word: 'orelha',
    enScope:
      'Portuguese **orelha** (ear): curiosity idiom **“pulga atrás da orelha”** and lab craft **[the ear glues what the mouth joined](' +
      H.orelhaCola +
      ')**. Case: [cigarro](' +
      H.cigarro +
      ') × [cigarra](' +
      ...
      H.sinais +
      ').',
    enSignal: 'curiosity (flea) + glue (ear-trap)',
    enStatus: 'ear · flea idiom · glue · cigarro ≠ cigarra',
    esScope:
      '**Orelha** (oreja): **«pulga atrás da orelha»** y el oficio **[la oreja pega lo que la boca juntó](' +
      H.orelhaCola +
      ')**. Caso: [cigarro](' +
      H.cigarro +
      ') × [cigarra](' +
      ...
      H.sinais +
      ').',
    esSignal: 'curiosidad (pulga) + pegamento (trampa de oído)',
    esStatus: 'oreja · pulga · cola · cigarro ≠ cigarra'
  });
  return { body, contentEn, contentEs, wiki };
}
