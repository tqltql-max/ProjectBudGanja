...
function isAdminOnlyHtml(pageFile) {
  const normalized = String(pageFile || '').replace(/\\/g, '/').replace(/^\/+/, '').toLowerCase();
  if (!normalized) return false;
  if (normalized === 'biblioteca/inspecoes/index.html' || normalized === 'biblioteca/inspecoes') return true;
  if (normalized === 'guia/cultivo-basico.html') return true;
  if (normalized.startsWith('posts/post-inspecao-') && normalized.endsWith('.html')) return true;
  return false;
}
