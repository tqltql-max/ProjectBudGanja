...
function isExpressaoInspecaoPost(p) {
  if (!p) return false;
  ...
  return (
    ...
    series.indexOf('expressoes') === 0 ||
    /^inspecao-expressao-/i.test(slug) ||
    /^inspecao-ditado-/i.test(slug)
  );
}
...
function toPublicFeedItem(p) {
  return {
    ...
    titleEs: p.titleEs || p.title_es || '',
    excerpt: p.excerpt || '',
    excerptEn: p.excerptEn || p.excerpt_en || '',
    excerptEs: p.excerptEs || p.excerpt_es || '',
    hasBodyEn: !!(p.contentEn || p.content_en),
    hasBodyEs: !!(p.contentEs || p.content_es),
    slug: p.slug,
...
function buildPostHtml(post) {
  ...
  let bodyHtmlPt = processContent(post.content_raw || post.content || '', format);
  bodyHtmlPt = appendPostExtras(post, bodyHtmlPt);
  const contentEn = String(post.contentEn || post.content_en || '').trim();
  const contentEs = String(post.contentEs || post.content_es || '').trim();
  let bodyHtmlEn = contentEn ? processContent(contentEn, format) : '';
  let bodyHtmlEs = contentEs ? processContent(contentEs, format) : '';
  if (bodyHtmlEn) bodyHtmlEn = appendPostExtras(post, bodyHtmlEn);
  if (bodyHtmlEs) bodyHtmlEs = appendPostExtras(post, bodyHtmlEs);
  const hasBodyEn = !!bodyHtmlEn;
  const hasBodyEs = !!bodyHtmlEs;
  const i18nNoteHtml = buildI18nNoteHtml(post, { hasBodyEn, hasBodyEs }) || '';
...
<div class="post-content" data-post-body data-locale="pt-BR" lang="pt-BR">${bodyHtmlPt}</div>
${hasBodyEn ? `<div class="post-content" data-post-body data-locale="en" lang="en" hidden>${bodyHtmlEn}</div>` : ''}
${hasBodyEs ? `<div class="post-content" data-post-body data-locale="es" lang="es" hidden>${bodyHtmlEs}</div>` : ''}
</main>
...
${
  pageType === 'inspecao' || String(slug || '').indexOf('inspecao-') === 0
    ? `<script src="/js/learn-glossary.js?v=${ASSET_VERSION}"></script>
<script src="/js/learn-translate.js?v=${ASSET_VERSION}"></script>`
    : ''
}
</body>
</html>`;
