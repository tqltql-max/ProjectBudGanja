...
function isInspectionPost(post) {
  return String((post && post.category) || '') === 'inspecao';
}

function isPubliclyListedPost(post) {
  if (!post || post.published === false) return false;
  if (isInspectionPost(post)) return false;
  return true;
}

function getPublicPosts(posts, category) {
  let list = (posts || []).filter(isPubliclyListedPost);
  if (category) list = list.filter((p) => (p.category || 'pesquisa') === category);
  return sortPublicPosts(list);
}

function getAdminListedPosts(posts, category) {
  let list = (posts || []).filter((p) => p && p.published !== false);
  if (category) list = list.filter((p) => (p.category || 'pesquisa') === category);
  return sortPublicPosts(list);
}
