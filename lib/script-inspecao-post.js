...
Inspeção editorial da palavra **[script](${self})** — empréstimo inglês vivo no português do Brasil para a **sequência escrita** que alguém (ou um interpretador) **segue**. Esta ficha cobre o **objeto** (lat. *scrīptum* ← *scrībere* «escrever»), as camadas **código / roteiro / letra cursiva / tag HTML**, o contraste com **roteiro** e **escrito**, a **correção BudGanja** (script ≠ destino, ≠ [skill](${skill})), e o fecho [Valeu !!!](${mantra}).

> **Nota metodológica:** auditoria independente. Fontes: [Wiktionary · script](${WIKT}), [script (PT)](${WIKT_PT}), [scrīptum](${WIKT_LAT}), [scripting language](${WIKI}). **Ficha ≠ curso de programação, ≠ manual de roteiro, ≠ protocolo de automação ofensiva.** Empréstimo consciente, como [skill](${skill}), [pattern](${pattern}) e [commitar](${commitar}).
...
| Elo código | [pattern](${pattern}) (molde) · [skill](${skill}) (craft) · [commitar](${commitar}) (rasto) |
...
| **Ofício lab** | Pasta \`scripts/\` deste site: gerar ficha, capa, upsert | Alta (mapa BudGanja) |
...
**H1:** objeto = lat. *scrīptum* ← *scrībere* → EN *script* → BR (código, palco, oral).  
**H2:** parece destino/mágica; é sequência escrita com autor.  
**H3:** irmãs = **roteiro** / **escrito**; elos = [skill](${skill}) · [pattern](${pattern}) · [commitar](${commitar}).  
...
**Aprovado** — **script** fichado: *scrīptum*; sequência escrita EN→BR; ≠ destino; elos [skill](${skill}) · [pattern](${pattern}) · [commitar](${commitar}); elo [Valeu !!!](${mantra}).

[▶ Palavras](${hub}) · [▶ Skill](${skill}) · [▶ Pattern](${pattern}) · [▶ Commitar](${commitar}) · [▶ Todas as inspeções](${hubAll}) · [▶ Valeu !!!](${mantra})
`;

  const contentEn = `## Scope

Editorial inspection of **[script](${self})** — English loan alive in Brazilian Portuguese for a **written sequence** that a person (or an interpreter) **follows**. Covers Latin *scrīptum* ← *scrībere*, layers **code / screenplay / HTML tag**, contrast with PT **roteiro** / **escrito**, **BudGanja correction** (script ≠ destiny, ≠ [skill](${skill})), and [Valeu !!!](${mantra}).
...
| Links | [skill](${skill}) · [pattern](${pattern}) · [commitar](${commitar}) · [Valeu !!!](${mantra}) |
| Date | ${inspected} |
...
**Approved** — EN loan → BR; written sequence; anti-destiny; links [skill](${skill}) · [pattern](${pattern}) · [commitar](${commitar}); [Valeu !!!](${mantra}).

[▶ Words](${hub}) · [▶ Skill](${skill}) · [▶ Pattern](${pattern}) · [▶ Commitar](${commitar}) · [▶ Valeu !!!](${mantra})
`;

  const contentEs = `## Alcance

Inspección de **[script](${self})** — préstamo inglés vivo en el portugués de Brasil para la **secuencia escrita** que alguien (o un intérprete) **sigue**. Cubre lat. *scrīptum* ← *scrībere*, capas **código / guion / etiqueta HTML**, contraste con **roteiro** / **escrito**, **corrección BudGanja** (script ≠ destino, ≠ [skill](${skill})) y [¡Valeu !!!](${mantra}).
...
| Vínculos | [skill](${skill}) · [pattern](${pattern}) · [commitar](${commitar}) · [¡Valeu !!!](${mantra}) |
| Fecha | ${inspected} |
...
**Aprobada** — préstamo EN→BR; secuencia escrita; anti-destino; vínculos [skill](${skill}) · [pattern](${pattern}) · [commitar](${commitar}); [¡Valeu !!!](${mantra}).

[▶ Palabras](${hub}) · [▶ Skill](${skill}) · [▶ Pattern](${pattern}) · [▶ Commitar](${commitar}) · [▶ ¡Valeu !!!](${mantra})
`;
...
function buildScriptPost() {
  ...
  return makePalavra({
    ...
    excerpt:
      'Palavras: «script» (lat. scrīptum ← scrībere) — sequência escrita EN→BR; roteiro × código; ≠ destino; elos skill/pattern/commitar; Valeu !!!',
    excerptEn:
      'Words: “script” (Lat. scrīptum ← scrībere) — written sequence EN→BR; roteiro × code; ≠ destiny; links skill/pattern/commitar; Valeu !!!',
    excerptEs:
      'Palabras: «script» (lat. scrīptum ← scrībere) — secuencia escrita EN→BR; roteiro × código; ≠ destino; vínculos skill/pattern/commitar; ¡Valeu !!!',
