...
function buildTheMatrixBodies() {
  const inspected = '2026-08-01';
  const hub = '/biblioteca/inspecoes/#inspecoes-artes';
  const wiki = 'https://pt.wikipedia.org/wiki/Matrix_(filme)';
...
Inspeção editorial do filme **«The Matrix»** (*Matrix*, 1999) — longa de ficção científica/ação das irmãs Wachowski, com **Keanu Reeves** no papel de Neo. O recorte é a **obra cinematográfica** (tese cultural, linguagem visual, impacto) — **não** a biografia do protagonista (já em [Keanu Reeves](${keanu}), série Pessoas).

> **Nota metodológica:** auditoria independente. Fontes: [Wikipédia (PT)](${wiki}), [Wikipedia (EN)](${wikiEn}), trailer oficial no YouTube (${yt}). Crédito da obra: Warner Bros. / Village Roadshow / Wachowski. Sem afiliação com o estúdio ou o elenco. **Não confundir** com Canais (YouTube), com [Legado](${legado}) canábico nem com a ficha Pessoas. A metáfora cultural da «pílula» é **simbólica** (acordar / escolher ver) — o laboratório **não** romantiza consumo de substâncias.
...
| Tipo BudGanja | Arte — cinema · tese da realidade simulada |
| Elo Pessoas | [Keanu Reeves](${keanu}) — actor (biografia / método; não duplicar aqui) |
| Elo laboratório | Método BudGanja: **verificar** o que se apresenta como real (inspeção ≠ aceitar a simulação) |
| Fonte de partida | [Wikipédia · Matrix (filme)](${wiki}) |
| Data da inspeção | ${inspected} |

## Hipóteses e método

**H1:** o valor BudGanja de *Matrix* é **epistemológico e cultural**: o filme dramatiza a pergunta «o que é real?» — a mesma disciplina que a inspeção exige perante canais, cursos e narrativas.  
...
Passos (variante «filme» na série Artes):

1. Identificar título, ano, realização, elenco âncora e fonte pública.  
2. Declarar a **tese cultural** (aqui: simulação vs verificação).  
3. Ligar a Pessoas só por referência (actor ≠ obra).  
4. Embed do trailer oficial.  
5. Status + fila.

## A obra (síntese)

- Hacker Thomas Anderson / Neo descobre que a vida quotidiana é uma simulação gerida por máquinas; junta-se à resistência liderada por Morpheus.  
- Inovação visual: *bullet time*, estilização de artes marciais, estética *cyberpunk* / *noir* digital.  
- Impacto: franquia (reloads, *Resurrections*), memes e léxico («red pill», «glitch in the Matrix»).  
- Prémios: 4 Óscares técnicos (1999) — efeitos, edição de som, som, montagem de efeitos (registo wiki).  
- Tom: escolha, treino, dúvida metódica — «ninguém pode ser *dito* o que é a Matrix; tem de ver por si».
...
| Recurso | Papel |
|---------|-------|
| [Keanu Reeves](${keanu}) | Biografia e método actoral — série [Pessoas](${pessoas}) |
...
- Abrir [Keanu Reeves](${keanu}) (Pessoas) depois desta ficha.  
...
1. Escolher o **filme** (título + ano), não a filmografia inteira do actor.  
2. Separar obra de biografia.  
3. Declarar tese cultural útil ao laboratório (sem forçar elo canábico).  
4. Slug \`inspecao-filme-…\` ou \`inspecao-arte-…\`.

## Status

**Aprovado na série Artes** — *The Matrix* (1999) documentado como obra; biografia do actor em [Keanu Reeves](${keanu}).
`;

  const contentEn = `## Scope

Editorial inspection of **The Matrix** (1999) — Wachowski sci-fi/action film starring **Keanu Reeves** as Neo. Focus is the **cinematic work** — **not** the lead’s biography (see [Keanu Reeves](${keanu}), People series).

> **Method note:** independent audit. Sources: [Wikipedia (EN)](${wikiEn}), official trailer (${yt}). Credit: Warner Bros. / Village Roadshow / Wachowski. No affiliation. The “pill” metaphor is **symbolic** (choose to see) — the lab does **not** romanticize substance use. Distinct from Channels, cannabis Legacy, and People sheets.
...
| Anchor cast | Keanu Reeves (Neo), Laurence Fishburne, Carrie-Anne Moss, Hugo Weaving |
| Medium | Feature film · science fiction / action |
| BudGanja type | Art — cinema · simulated-reality thesis |
| People link | [Keanu Reeves](${keanu}) |
| Date | ${inspected} |

## Hypotheses

**H1:** cultural/epistemic value — the film stages “what is real?” as inspection discipline.  
**H2:** Neo is the face of the **work**; the actor’s craft stays in People.  
**H3:** “red/blue pill” is treated as a metaphor of informed choice, not pharmacology.

## The work (summary)

Hacker Neo learns everyday life is a machine simulation; joins Morpheus’s resistance. Visual landmarks: bullet time, martial-arts styling, cyberpunk palette. Franchise and meme lexicon followed. Four technical Oscars (per wiki).

## Lab thesis

| Film theme | Editorial gloss |
|------------|-----------------|
| Convincing simulation | Narrative that passes as truth without proof |
| Pill / choice | Inspect vs accept the given |
| Training | Repeatable method; hypothesis + trial |
...
**Approved in the Arts series** — *The Matrix* (1999) as work; actor biography in [Keanu Reeves](${keanu}).
`;

  const contentEs = `## Alcance

Inspección editorial de **The Matrix** (*Matrix*, 1999) — largometraje de ciencia ficción/acción de las Wachowski, con **Keanu Reeves** como Neo. El recorte es la **obra** — **no** la biografía del protagonista ([Keanu Reeves](${keanu}), serie Personas).

> **Nota metodológica:** auditoría independiente. Fuentes: [Wikipedia](${wikiEn}), tráiler oficial (${yt}). Crédito: Warner Bros. / Village Roadshow / Wachowski. La metáfora de la «píldora» es **simbólica** — el laboratorio **no** romantiza el consumo de sustancias.
...
| Elenco ancla | Keanu Reeves (Neo), Laurence Fishburne, Carrie-Anne Moss, Hugo Weaving |
| Medio | Largometraje · ciencia ficción / acción |
| Tipo BudGanja | Arte — cine · tesis de la realidad simulada |
| Personas | [Keanu Reeves](${keanu}) |
| Fecha | ${inspected} |

## Hipótesis

**H1:** valor epistemológico/cultural — «¿qué es real?» como disciplina de inspección.  
**H2:** Neo es el rostro de la **obra**; el oficio del actor queda en Personas.  
**H3:** «píldora» = metáfora de elección informada, no consejo farmacológico.
...
**Aprobada en la serie Artes** — *The Matrix* (1999) como obra; biografía del actor en [Keanu Reeves](${keanu}).
`;
...
function buildTheMatrixPost() {
  ...
  return artePost({
    title: 'Inspeção: The Matrix — cinema, simulação e o método de verificar',
    titleEn: 'Inspection: The Matrix — cinema, simulation and the method of verifying',
    titleEs: 'Inspección: The Matrix — cine, simulación y el método de verificar',
    excerpt:
      'Artes · cinema: The Matrix (1999, Wachowski) — obra sobre realidade simulada e escolha de ver; distinta da biografia em Keanu Reeves (Pessoas).',
    excerptEn:
      'Arts · film: The Matrix (1999, Wachowski) — work on simulated reality and choosing to see; distinct from the Keanu Reeves biography (People).',
    excerptEs:
      'Artes · cine: The Matrix (1999, Wachowski) — obra sobre realidad simulada y elegir ver; distinta de la biografía en Keanu Reeves (Personas).',
