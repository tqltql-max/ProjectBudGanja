...
const { handleGoogleStart, decodeOAuthReturnTo, verifyOAuthState } = require('./auth-google-start.js');
...
async function handleGoogleAuth(req, options) {
  ...
  if (path === '/api/auth/google/callback' && method === 'GET') {
    ...
    const stateOk = !!(state && savedState)
    ...
    const returnTo = savedState
      ...
      : (signedState ? signedState.returnTo : decodeOAuthReturnTo(state));
