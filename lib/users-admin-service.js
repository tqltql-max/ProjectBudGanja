...
async function deleteUserForAdmin(store, userId, actor) {
  if (!store.deleteUser || !store.getUserByIdAdmin) {
    return { ok: false, status: 501, error: 'Gestão de utilizadores indisponível neste backend.' };
  }
  const id = String(userId || '').trim();
  if (!id) return { ok: false, status: 400, error: 'ID inválido.' };

  const target = await store.getUserByIdAdmin(id);
  if (!target) return { ok: false, status: 404, error: 'Utilizador não encontrado.' };

  if (actor && actor.userId && String(actor.userId) === id) {
    return { ok: false, status: 409, error: 'Não pode remover a sua própria conta enquanto está autenticado.' };
  }

  if (target.isAdmin) {
    const dbAdmins = await store.countAdminUsers();
    const envAdmins = getAdminEmails().size;
    if (dbAdmins <= 1 && !envAdmins) {
      return { ok: false, status: 409, error: 'Não é possível remover o último administrador.' };
    }
  }

  const deleted = await store.deleteUser(id);
  if (!deleted) return { ok: false, status: 500, error: 'Não foi possível remover a conta.' };
