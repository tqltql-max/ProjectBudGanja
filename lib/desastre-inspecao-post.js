...
> **Nota metodológica:** auditoria independente. Fontes: [Wikcionário · desastre](${WIKT}), [Wikipédia · Desastre](${WIKI}). **Ficha ≠ boletim de emergência, ≠ teologia do azar, ≠ protocolo clínico.** [Etimologia](${etimologia}) popular (*desatar* + resto) = mecanismo de [trocadilho](${trocadilho}) / história que encaixa demais — **não** é o étimo. Fecho: [Valeu !!!](${mantra}).
...
5. Depois: **[ufa](${ufa})** — e [Valeu !!!](${mantra}).
...
[▶ Palavras](${hub}) · [▶ Desatar](${desatar}) · [▶ Desatar o nó](${desatarNo}) · [▶ Nó](${no}) · [▶ Valeu !!!](${mantra})
`;

  const contentEn = `## Scope

Portuguese **desastre** (disaster) from *dis-* + *astrum* (ill-starred). **Not** [desatar](${desatar}) (to untie). The requested link is **craft**: a [nó na vida!](${noVida}) left unnamed can be lived as disaster; [desatar o nó](${desatarNo}) is the gesture. Close: [Valeu !!!](${mantra}).

## Status

**Approved** — disaster = bad star; untying = another etymon; related by method, not by ancestor.

[▶ Words](${hub}) · [▶ Untie](${desatar}) · [▶ Untie the knot](${desatarNo}) · [▶ Valeu !!!](${mantra})
`;

  const contentEs = `## Alcance

**Desastre** de *dis-* + *astrum* (mala estrella). **No** es [desatar](${desatar}). El vínculo pedido es **oficio**: un [nó na vida!](${noVida}) sin nombrar se vive como desastre; [desatar o nó](${desatarNo}) es el gesto. Cierre: [¡Valeu !!!](${mantra}).

## Estado

**Aprobado** — desastre = mala estrella; desatar = otro étimo; relación de método, no de abuelo.

[▶ Palabras](${hub}) · [▶ Desatar](${desatar}) · [▶ Desatar o nó](${desatarNo}) · [▶ ¡Valeu !!!](${mantra})
`;
...
function buildDesastrePost(seriesOrder) {
  ...
  return makePalavra({
    ...
    excerpt:
      'Palavras: «desastre» (*dis-* + *astrum*) — má estrela; ≠ desatar; relação de ofício com desatar o nó; ufa depois; Valeu !!!',
    excerptEn:
      'Words: “desastre” (*dis-* + *astrum*) — ill-starred; ≠ untying; craft link to desatar o nó; phew after; Valeu !!!',
    excerptEs:
      'Palabras: «desastre» (*dis-* + *astrum*) — mala estrella; ≠ desatar; vínculo de oficio con desatar o nó; ufa después; ¡Valeu !!!',
