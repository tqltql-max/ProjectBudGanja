...
const MITOLOGIA_PATH = path.join(ROOT, 'content', 'mitologia.json');

const CATEGORIES = [
  { id: 'lema', label: 'Lema' },
  { id: 'egito', label: 'Egito' },
  { id: 'grego', label: 'Grécia' },
...
function normalizeItem(raw) {
  ...
  return {
    ...
    category: String(raw.category || '').trim().toLowerCase() || 'lexico',
...
function readMitologia() {
  try {
    const data = JSON.parse(fs.readFileSync(MITOLOGIA_PATH, 'utf8') || '{}');
    const items = Array.isArray(data.items) ? data.items : [];
    return {
      updatedAt: data.updatedAt || null,
      disclaimer:
        data.disclaimer ||
        'Catálogo educacional de mitos e deuses. Não é culto nem magia.',
