...
Inspeção editorial da palavra **genial** — adjetivo (e interjeição de elogio no português do Brasil). Morfologia: [gênio](${wikiGenio}) + *-al*. Nomeia o que parece **nascido do engenho** — ideia luminosa, solução elegante, obra que acerta. No laboratório, «genial!» é festa legítima; **não** substitui o [gesto](${gesto}) nem o mantra [Faça o melhor!](${mantra}).
...
| Tipo BudGanja | Palavra — elogio de engenho; risco de flattery vazia ou culto do génio |
| Elo principal | [criatividade](${criatividade}) · [Faça o melhor!](${mantra}) |
...
3. Cruzar com criatividade, mantra e Senna (excelência com ofício).  
4. Status.
...
| Uso | Leitura | Elo |
|-----|---------|-----|
| «Genial!» + o quê acertou | Ofício — elogio com objecto | [verdade](${verdade}) · [gesto](${gesto}) |
| «Ideia genial» + teste / ficha | [Criatividade](${criatividade}) com método | [criatividade](${criatividade}) |
| «É genial» sem olhar | Flattery — apaga a inspeção | [aff](${aff}) · [já](${ja}) |
| «Não sou genial, então…» | Armadilha — o mantra não exige génio | [mantra](${mantra}) |
| Excelência na pista / no ofício | Engenho treinado, não só rótulo | [Senna](${senna}) · [caminho](${caminho}) |

**Veredicto:** o laboratório **gosta** de ouvir «genial» quando há **feito**. **Desconfia** quando a palavra fecha a ficha sem relatório.
...
| [Faça o melhor!](${mantra}) | Escala diária — melhor ≠ génio absoluto |
| [Gesto](${gesto}) · [Verdade](${verdade}) | Prova do elogio |
| [Ayrton Senna](${senna}) | Excelência brasileira com ofício |
| [Vida](${vida}) | Celebrar sem pieguice nem culto |

## Limites

- Não mede QI nem «talento inato».  
- Não exige genialidade para publicar inspeção.  
- Ingl. *genial* (amável) ≠ elogio BR.

## Status

**Aprovado** — «genial» fichada como elogio de engenho; uso com objecto e método sim; culto vazio / «já é genial» sem inspecionar = ressalva.

[▶ Palavras](${hub}) · [▶ Criatividade](${criatividade}) · [▶ Faça o melhor!](${mantra}) · [▶ Senna](${senna}) · [▶ Vida](${vida})
`;

  const contentEn = `## Scope

Word sheet for Portuguese **genial** — adjective and praise interjection in Brazilian use (*gênio* + *-al*, from Lat. *genius*). Names recognized ingenuity. In the lab, “genial!” is fair celebration; it does **not** replace [gesture](${gesto}) or [Do your best!](${mantra}).

> Method note: [Wiktionary · genial](${wiki}). Partial false friend with English *genial* (“amiable / mild weather”). Word sheet ≠ IQ cult.
...
| Use | Reading |
|-----|---------|
| “Genial!” + what worked | Craft — praise with an object |
| “Genial” without looking | Flattery — skips inspection |
| “I’m not a genius, so…” | Trap — the mantra does not require genius |

**Verdict:** celebrate the deed; distrust the label that closes the sheet.

## Status

**Approved** — praise of ingenuity with method; empty cult = caveat.

[▶ Words](${hub}) · [▶ Creativity](${criatividade}) · [▶ Do your best!](${mantra}) · [▶ Senna](${senna})
`;

  const contentEs = `## Alcance

Ficha de **genial** — adjetivo e exclamación de elogio en el uso BR (*gênio* + *-al*, lat. *genius*). Nombra el ingenio reconocido. En el lab, «¡genial!» es fiesta legítima; **no** sustituye el [gesto](${gesto}) ni [¡Haz lo mejor!](${mantra}).

> Nota: [Wikcionario · genial](${wiki}). Falso amigo parcial con el inglés *genial* («amable / clima suave»). Ficha ≠ culto al genio innato.
...
| Uso | Lectura |
|-----|---------|
| «¡Genial!» + qué acertó | Oficio — elogio con objeto |
| «Genial» sin mirar | Halago — salta la inspección |
| «No soy genial, entonces…» | Trampa — el mantra no exige genio |

**Veredicto:** celebrar el hecho; desconfiar de la etiqueta que cierra la ficha.

## Estado

**Aprobada** — elogio de ingenio con método; culto vacío = salvedad.

[▶ Palabras](${hub}) · [▶ Creatividad](${criatividade}) · [▶ ¡Haz lo mejor!](${mantra}) · [▶ Senna](${senna})
`;
...
function buildGenialPost() {
  ...
  return palavraPost({
    title: 'Inspeção: Genial — engenho, elogio e o risco do culto vazio',
    titleEn: 'Inspection: Genial — ingenuity, praise and the risk of empty cult',
    titleEs: 'Inspección: Genial — ingenio, elogio y el riesgo del culto vacío',
    excerpt:
      'Palavras: «genial» (*gênio* + *-al*) — elogio de engenho no BR; com método sim; culto vazio / sem inspeção = ressalva; Faça o melhor! ≠ seja génio.',
    excerptEn:
      'Words: “genial” (*gênio* + *-al*) — BR praise of ingenuity; with method yes; empty cult = caveat; Do your best! ≠ be a genius.',
    excerptEs:
      'Palabras: «genial» (*gênio* + *-al*) — elogio BR de ingenio; con método sí; culto vacío = salvedad; ¡Haz lo mejor! ≠ sé un genio.',
