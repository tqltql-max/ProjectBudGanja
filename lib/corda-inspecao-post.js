...
> **Nota metodológica:** auditoria independente. Fontes: [Wikcionário · corda](${WIKT}), lat. [chorda](${WIKT_LA}) ← gr. χορδή. **Ficha ≠ manual de amarração, ≠ protocolo de força, ≠ partitura.** Irmã de suporte: [cinta](${cinta}) (faixa — outro objecto). Fecho: [Faça o melhor!](${mantra}).
...
| Não é | [nó](${no}) (o laço) · [codorna](${codorna}) (ave) · [cinta](${cinta}) (faixa) |
| Data | ${inspected} |
...
| **Música** | Corda de instrumento — mesmo avô grego |
| **Figura** | Corda da [vida](${noVida}) — o fio; o aperto é o [nó](${no}) |
| **Soltar** | [Desatar o nó](${desatarNo}) — ofício no objecto |

## 4. Faça o melhor!

Inspecionar a **corda** antes de culpar o [nó](${no}). Não confundir com [codorna](${codorna}). Não deixar o aperto virar [desastre](${desastre}) por falta de [desatar](${desatar}). Depois: **[ufa](${ufa})** e [Faça o melhor!](${mantra}).

## Status

**Aprovado** — **corda** fichada como objecto (*chorda*); mora no catálogo [Objetos](${objetos}); recebe o [nó](${no}); ≠ [codorna](${codorna}).

[▶ Palavras](${hub}) · [▶ Objetos](${objetos}) · [▶ Nó](${no}) · [▶ Codorna](${codorna}) · [▶ Faça o melhor!](${mantra})
`;

  const contentEn = `## Scope

Portuguese **corda** (rope/cord) from Latin *chorda* / Greek χορδή — the **object** that holds a [nó](${no}) (knot). Not [codorna](${codorna}) (quail). Catalog: [Objetos](${objetos}). Close: [Do your best!](${mantra}).

## Status

**Approved** — rope as thing; knot lives on it; quail is another etymon.

[▶ Words](${hub}) · [▶ Objects](${objetos}) · [▶ Knot](${no}) · [▶ Do your best!](${mantra})
`;

  const contentEs = `## Alcance

**Corda** (cuerda) del lat. *chorda* / gr. χορδή — el **objeto** donde vive el [nó](${no}). No es [codorna](${codorna}). Catálogo: [Objetos](${objetos}). Cierre: [¡Haz lo mejor!](${mantra}).

## Estado

**Aprobada** — cuerda como cosa; el nudo vive en ella; la codorniz es otro étimo.

[▶ Palabras](${hub}) · [▶ Objetos](${objetos}) · [▶ Nó](${no}) · [▶ ¡Haz lo mejor!](${mantra})
`;
...
function buildCordaPost(seriesOrder) {
  ...
  return makePalavra({
    ...
    excerpt:
      'Palavras-objecto: «corda» (lat. *chorda*) — fio que recebe o nó; ≠ cinta ≠ codorna; catálogo Objetos; Faça o melhor!',
    excerptEn:
      'Object-word: “corda” (Lat. *chorda*) — rope that takes the knot; ≠ tape ≠ quail; Objects catalog; Do your best!',
    excerptEs:
      'Palabra-objeto: «corda» (lat. *chorda*) — cuerda que recibe el nudo; ≠ cinta ≠ codorniz; catálogo Objetos; ¡Haz lo mejor!',
