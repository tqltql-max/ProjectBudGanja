...
const {
  ...
  writeThemesJson: writeDalleThemesJson,
...
function buildLairRibeiroBodies(opts) {
  opts = opts || {};
...
function buildLairRibeiroPost(opts) {
  opts = opts || {};
  // Avoid writing themes JSON on every module load during site build.
  const writeJson = opts.writeThemesJson === true;
  const { body, contentEn, contentEs, liveCannabis } = buildLairRibeiroBodies({
    writeThemesJson: writeJson
  });
...
function buildDalleLasteBodies(opts) {
  opts = opts || {};
  const inspected = '2026-08-25';
  const hub = '/biblioteca/inspecoes/#inspecoes-divulgacao';
  const site = 'https://drsamueldallelaste.com.br/';
  const clinic = 'https://clinicadallelaste.com.br/';
  const yt = 'https://www.youtube.com/@DrSamuelDalleLaste';
  const ytLegacy = 'https://www.youtube.com/c/DrSamuelDalleLaste';
  const ytId = 'UCQ-mdo78ry4nzEZ2HdafajQ';
  const caras =
    'https://caras.com.br/caras-mais/escolher-uma-longevidade-consciente.phtml';
  const liveCannabis = 'AU6hm4drFO4';
  const seedVideo = '288-hT2SI-4';
  const featuredId = FEATURED_VIDEO_ID;
  const featuredTitle = FEATURED_VIDEO_TITLE;
  const featuredSummary = VIDEO_SUMMARIES[featuredId] || {};
  const videosHub = '/videos/?channel=dallelaste&v=' + featuredId;
  const planta = '/plantas/cannabis-sativa/';
  const palavras = '/posts/post-inspecao-palavra-maconha.html';
  const unifesp = '/posts/post-inspecao-curso-unifesp-cannabis-medicinal.html';
  const carlini = '/posts/post-inspecao-elisaldo-carlini.html';
  ...
  const canabinall = '/posts/post-inspecao-canal-canabinall.html';
  const legado = '/biblioteca/inspecoes/#inspecoes-pessoas';
  const dalleThemes = '/content/channels/dalle-video-themes.json';
  const dalleCatalog = '/content/channels/drsamueldallelaste.json';

  const themesDoc = categorizeDalleVideos();
  if (opts.writeThemesJson === true) {
    writeDalleThemesJson(themesDoc);
  }
  const catalogMd = buildDalleCatalogMarkdown({ doc: themesDoc });
  const catalogEn = buildDalleCatalogSummaryI18n('en', themesDoc);
  const catalogEs = buildDalleCatalogSummaryI18n('es', themesDoc);
  const n = themesDoc.categorizedCount;
  const dicasN = (themesDoc.themes.find((t) => t.id === 'dicas') || {}).count || 0;
  const pergunteN = (themesDoc.themes.find((t) => t.id === 'pergunte') || {}).count || 0;
  const podcastsN = (themesDoc.themes.find((t) => t.id === 'podcasts') || {}).count || 0;

  const body = `## Escopo

Inspeção editorial de **Dr. Samuel Dalle Laste** (CRM-RS **32011**) — médico, diretor técnico da [Clínica Dalle Laste](${clinic}) (Porto Alegre) e criador do **canal de divulgação** no YouTube — e do **acervo completo de títulos** do canal. Série **Divulgação** — complementar ao ecossistema; **não é origem UNIFESP / MovReCam / CEBRID**.

> **Nota metodológica:** auditoria independente. Fontes: [portal drsamueldallelaste.com.br](${site}), [Clínica Dalle Laste](${clinic}), [CARAS · longevidade](${caras}), canal YouTube verificado por ID. **Não é aconselhamento médico.** Sem afiliação com o autor, a clínica, o CNPJ de conteúdo digital ou o canal. **Não equiparar** esta ficha a [Carlini](${carlini}), [Ticão/curso UNIFESP](${unifesp}) ou [CANABinALL](${canabinall}). Indexar ≠ endossar medicina integrativa, ortomolecular ou protocolos de longevidade.

## Objeto inspecionado

| Campo | Valor |
|-------|-------|
| Nome | **Samuel Dalle Laste** |
| CRM citado no portal | CRM-RS 32011 |
| Tipo BudGanja | Divulgação de saúde — pessoa + canal |
| Clínica (portal) | [Clínica Dalle Laste](${clinic}) — R. Tobias da Silva, 120, Porto Alegre/RS |
| Conteúdo digital (portal) | SDL Conteúdo Digital LTDA · CNPJ 39.327.221/0001-04 |
| Elo planta | [Cannabis sativa](${planta}) |
| Elo Palavras | [maconha](${palavras}) |
| Data da inspeção | ${inspected} |
...
| Campo | Valor |
|-------|-------|
| Nome no YouTube | **Dr. Samuel Dalle Laste** |
| Handle | [@DrSamuelDalleLaste](${yt}) |
| ID do canal | \`${ytId}\` |
| URL canónica | [youtube.com/channel/${ytId}](https://www.youtube.com/channel/${ytId}) |
| URL vanity | [${yt}](${yt}) |
| URL clássica (/c/) | [${ytLegacy}](${ytLegacy}) |
| Site oficial | [${site}](${site}) |
| Clínica | [${clinic}](${clinic}) |
| Destaque no hub | [${featuredTitle}](${videosHub}) |

### Destaque — O que realmente PREVINE INFARTO

| Campo | Valor |
|-------|-------|
| Título | ${featuredTitle} |
| ID | \`${featuredId}\` |
| URL | [youtube.com/watch?v=${featuredId}](https://www.youtube.com/watch?v=${featuredId}) |
| Hub | [/videos/?channel=dallelaste&v=${featuredId}](${videosHub}) |
| Tema | Coração, pressão e vasos |
| Estudo citado (números do vídeo) | Get With The Guidelines · Sachdeva et al., *Am Heart J* 2009 (no vídeo: «Java») |

@youtube ${featuredId}

${featuredSummary.summary}

> **Caderno:** um parágrafo único — ofício da aula, não receita. Assistir no site: [destaque no hub de vídeos](${videosHub}).

### Vídeos de referência (cannabis no canal)

| Campo | Valor |
|-------|-------|
| Título | Afinal, MACONHA faz mal pra saúde? |
| ID | \`${liveCannabis}\` |
| URL | [youtube.com/watch?v=${liveCannabis}](https://www.youtube.com/watch?v=${liveCannabis}) |
...
| Campo | Valor |
|-------|-------|
| Título | Semente da Maconha: uma boa opção de proteína? |
| ID | \`${seedVideo}\` |
| URL | [youtube.com/watch?v=${seedVideo}](https://www.youtube.com/watch?v=${seedVideo}) |

> **Nota de catálogo:** o canal mistura prevenção, bioquímica didáctica, longevidade, ortomolecular e temas clínicos em tom de divulgação. Marcos canábicos: vídeos acima. **Categorização completa (${inspected}):** **${n}** títulos únicos (crawl do acervo; o YouTube público cita ~866) em temas editoriais — séries numeradas agrupam os **vídeos iguais demais** (**DICA # / VERÃO #:** ${dicasN}; **Pergunte ao Dr. Samuel:** ${pergunteN}; **Olá Pessoal / podcasts:** ${podcastsN}) → [\`dalle-video-themes.json\`](${dalleThemes}) e secção *Catálogo temático* abaixo. Catálogo bruto: [\`drsamueldallelaste.json\`](${dalleCatalog}).

## Hipóteses e método

**H1:** o valor BudGanja é mapear **onde** o canal fala de maconha/cannabis e **como** o acervo se agrupa em séries repetidas (iguais demais), distinto do eixo académico já inspecionado.  
**H2:** o portal e a clínica falam de **medicina integrativa / prevenção / longevidade** — divulgação comercial-didáctica, não substituto de artigo peer-reviewed do laboratório.  
**H3:** agrupar DICA #, VERÃO #, Pergunte e Olá Pessoal evita que o hub trate cada título curto como objecto novo.  
**H4:** o destaque **PREVINE INFARTO** e o tom «faz mal / faz bem» da âncora de maconha são **divulgação**; o laboratório cruza com [planta](${planta}) e [Palavras](${palavras}), sem validar o veredicto clínico do vídeo nem mandar parar estatina.

Passos:

1. Confirmar IDs do canal (handle + UC… + URL /c/).  
2. Escolher vídeos-âncora de maconha no canal.  
3. Varrer o acervo e classificar **um balde por vídeo** (primeira palavra-chave).  
4. Isolar séries numeradas (iguais demais).  
5. Cruzar com [planta](${planta}) / [Palavras](${palavras}) / [Lair](${lair}) / [Davis](${davis}) (por contraste).  
6. Declarar limites (integrativa ≠ UNIFESP).  
7. Status.

## Quem é (síntese pública)

- Médico (CRM-RS 32011, citado no [portal](${site})); diretor técnico da [Clínica Dalle Laste](${clinic}).  
- O portal descreve abordagem de **prevenção** e **medicina integrativa**, com ênfase em estilo de vida e longevidade; também empresário, palestrante e criador do canal.  
- [CARAS](${caras}) cita pós-graduações em nutrologia, ortomolecular, ciências da longevidade e medicina funcional, e atendimento em Porto Alegre e São Paulo — **crédito à reportagem**; não auditámos currículos internos aqui.  
- Canal criado em 15 jan. 2018 (Education); ordem de grandeza pública ~2,3 M de inscritos.

## Limites e controvérsias de método

- **Não** é ficha de Legado: não há elo institucional com [UNIFESP](${unifesp}) / MovReCam / CEBRID.  
- Medicina integrativa, ortomolecular, hormese, protocolos de exame e fármacos citados nos títulos são **objecto de catálogo**, não endosso.  
- O laboratório não reproduz posologia, não valida Ozempic/Mounjaro nem «curas naturais».  
- Contraste: [Lair](${lair}) (divulgação BR + açúcar/cana) e [Davis](${davis}) (divulgação anti-trigo) — três eixos, três fichas.
...
|------|---------|
| Hub Divulgação | [Inspeções · Divulgação](${hub}) |
| Catálogo temático (todos) | [\`dalle-video-themes.json\`](${dalleThemes}) |
| Catálogo bruto | [\`drsamueldallelaste.json\`](${dalleCatalog}) |
| Hub de vídeos (destaque) | [/videos/?channel=dallelaste&v=${featuredId}](${videosHub}) |
| Planta | [Cannabis sativa](${planta}) |
| Palavra | [maconha](${palavras}) |
| Outra divulgação BR | [Lair Ribeiro](${lair}) |
| Divulgação anti-trigo | [William Davis](${davis}) |
| Legado (contraste) | [Inspeções · Legado](${legado}) |
| Extensão académica | [Curso UNIFESP](${unifesp}) |
...
**Aprovado na série Divulgação** — canal indexado por ID; destaque [PREVINE INFARTO](${videosHub}) com parágrafo-caderno no hub; âncoras de maconha; classificação exclusiva dos ${n} títulos. Indexar ≠ endossar.

[▶ Divulgação](${hub}) · [Destaque no hub](${videosHub}) · [Planta](${planta}) · [Canal](${yt}) · [Site](${site})
`;

  const contentEn = `## Scope

Editorial inspection of **Dr. Samuel Dalle Laste** (CRM-RS **32011**) and the **full title archive** of his YouTube channel. **Outreach** series — not UNIFESP / MovReCam / CEBRID origin.

> **Method note:** independent audit. Sources: [portal](${site}), [clinic](${clinic}), [CARAS](${caras}), YouTube channel ID. **Not medical advice.** Indexing ≠ endorsing integrative / orthomolecular / longevity claims.

## Channel IDs (verified)

| Field | Value |
|-------|-------|
| Name | Dr. Samuel Dalle Laste |
| Handle | [@DrSamuelDalleLaste](${yt}) |
| ID | \`${ytId}\` |
| Site | [${site}](${site}) |
| Clinic | [${clinic}](${clinic}) |
| Featured | [What really PREVENTS heart attack](${videosHub}) |

@youtube ${featuredId}

${featuredSummary.summaryEn}

> **Notebook:** one paragraph — the lesson’s craft, not a prescription. Watch on-site: [videos hub highlight](${videosHub}).

**Catalog (${inspected}):** **${n}** unique titles. Look-alike numbered series: TIP # / SUMMER # (${dicasN}), Ask Dr. Samuel (${pergunteN}), Olá Pessoal / podcasts (${podcastsN}) → [\`dalle-video-themes.json\`](${dalleThemes}).

## Limits

Do **not** equate with [Carlini](${carlini}) or [UNIFESP](${unifesp}). Contrast: [Lair](${lair}), [Davis](${davis}).

${catalogEn}

## Status

**Approved as Outreach sheet** — channel indexed by ID; cannabis anchors; exclusive classification of ${n} titles, with numbered series grouping look-alike videos.

[▶ Outreach](${hub}) · [Plant](${planta}) · [Channel](${yt})
`;

  const contentEs = `## Alcance

Inspección editorial del **Dr. Samuel Dalle Laste** (CRM-RS **32011**) y del **archivo completo de títulos** de su canal. Serie **Divulgación** — no es origen UNIFESP / MovReCam / CEBRID.

> **Nota metodológica:** auditoría independiente; catálogo temático → [\`dalle-video-themes.json\`](${dalleThemes}). **No es consejo médico.** Indexar ≠ respaldar medicina integrativa.

## Referencias del canal (verificado)

| Campo | Valor |
|-------|-------|
| Nombre | Dr. Samuel Dalle Laste |
| Handle | [@DrSamuelDalleLaste](${yt}) |
| ID | \`${ytId}\` |
| Sitio | [${site}](${site}) |
| Clínica | [${clinic}](${clinic}) |
| Destacado | [Qué realmente PREVENE el infarto](${videosHub}) |

@youtube ${featuredId}

${featuredSummary.summaryEs}

> **Cuaderno:** un párrafo — oficio de la clase, no receta. Ver en el sitio: [destaque en el hub](${videosHub}).

**Catálogo (${inspected}):** **${n}** títulos. Series demasiado iguales: TIP # / VERANO # (${dicasN}), Pregunte al Dr. Samuel (${pergunteN}), Olá Pessoal / podcasts (${podcastsN}).

## Límites

No equiparar con [Carlini](${carlini}) ni [UNIFESP](${unifesp}). Contraste: [Lair](${lair}), [Davis](${davis}).

${catalogEs}

## Estado

**Aprobada en Divulgación** — canal indexado por ID; anclas de cannabis; clasificación exclusiva de ${n} títulos, con series numeradas que agrupan los vídeos demasiado iguales.

[▶ Divulgación](${hub}) · [Planta](${planta}) · [Canal](${yt})
`;

  return { body, contentEn, contentEs, featuredId, themesDoc };
}

function buildDalleLastePost(opts) {
  opts = opts || {};
  const writeJson = opts.writeThemesJson === true;
  const { body, contentEn, contentEs, featuredId } = buildDalleLasteBodies({
    writeThemesJson: writeJson
  });
  return divulgacaoPost({
    title: 'Inspeção: Dr. Samuel Dalle Laste — canal, infarto e vídeos iguais demais',
    titleEn: 'Inspection: Dr. Samuel Dalle Laste — channel, heart attack and look-alike videos',
    titleEs: 'Inspección: Dr. Samuel Dalle Laste — canal, infarto y vídeos demasiado iguales',
    excerpt:
      'Divulgação: destaque «O que realmente PREVINE INFARTO» (parágrafo-caderno no hub), IDs CRM-RS 32011 e catálogo exclusivo dos ~811 vídeos. Indexar ≠ endossar.',
    excerptEn:
      'Outreach: featured “What really PREVENTS heart attack” (notebook paragraph on the hub), CRM-RS 32011 IDs and an exclusive catalog of ~811 videos. Indexing ≠ endorsement.',
    excerptEs:
      'Divulgación: destaque «Qué realmente PREVENE el infarto» (párrafo-cuaderno en el hub), IDs CRM-RS 32011 y catálogo exclusivo de ~811 vídeos. Indexar ≠ respaldar.',
    slug: 'inspecao-divulgacao-samuel-dalle-laste',
    date: '2026-08-25T12:00:00.000Z',
    seriesOrder: 2,
    seriesLabel: 'Samuel Dalle Laste · divulgação',
    coverImage: 'https://i.ytimg.com/vi/' + featuredId + '/hqdefault.jpg',
    sourceUrl: 'https://www.youtube.com/@DrSamuelDalleLaste',
    videoId: featuredId,
    body,
    contentEn,
    contentEs
...
const DIVULGACAO_INSPECOES_POSTS = [
  buildDalleLastePost({ writeThemesJson: false }),
  buildLairRibeiroPost({ writeThemesJson: false })
];
