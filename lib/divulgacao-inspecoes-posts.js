...
> **Nota de catálogo:** o canal mistura nutrição, longevidade, autoajuda e temas clínicos em tom de divulgação. Marco canábico: vídeo acima. **Varredura açúcar/cana (2026-08-01):** aba Vídeos paginada até ao fim (**851** IDs) + **21** buscas no canal + RSS → **887** títulos únicos; **38** hits temáticos (açúcar, frutose, diabetes, adoçantes, metabólico…). Relatório: [\`lair-sugar-hits.json\`](${lairHits}).
...
**Aprovado como ficha fundadora da série Divulgação** — canal e textos canábicos documentados; cruzamento açúcar/cana (portal + varredura de 887 vídeos) ligado a [Derivados · cana](${cana}); independência e limites explícitos.
...
> **Method note:** independent audit; 887-video title scan → [\`lair-sugar-hits.json\`](${lairHits}). **Not medical advice.** Indexing ≠ endorsement.
...
**Approved as founding Outreach sheet** — with sugar/cane cross-link to Derivatives.
...
> **Nota metodológica:** auditoría independiente; barrido de 887 títulos → [\`lair-sugar-hits.json\`](${lairHits}). **No es consejo médico.** Indexar ≠ respaldar.
...
**Aprobada como ficha fundadora de Divulgación** — con cruce azúcar/caña.
...
function buildLairRibeiroPost() {
  ...
  return divulgacaoPost({
    ...
    excerpt:
      'Divulgação: IDs do canal, textos canábicos, limites públicos e varredura de 887 vídeos sobre açúcar/frutose/diabesidade — elo com a ficha Derivados da cana-de-açúcar (portal nomeia sacarose da cana).',
    excerptEn:
      'Outreach: channel IDs, cannabinoid texts, public limits, and a 887-video scan on sugar/fructose/diabesity — linked to the sugarcane Derivatives sheet (portal names cane sucrose).',
    excerptEs:
      'Divulgación: IDs del canal, textos cannábicos, límites públicos y barrido de 887 vídeos sobre azúcar/fructosa/diabesidad — vínculo con la ficha Derivados de la caña (el portal nombra la sacarosa de caña).',
