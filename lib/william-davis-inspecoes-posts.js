...
- Contraponto científico: [Brouns 2013](/posts/post-inspecao-artigo-brouns-trigo-obesidade-2013.html) · [Wieser 2020](/posts/post-inspecao-artigo-wieser-duas-faces-trigo-2020.html).
...
function buildWilliamDavisBodies(opts) {
  opts = opts || {};
  const inspected = '2026-08-02';
  ...
  const yt = 'https://www.youtube.com/@williamdavismd';
  const davisThemes = '/content/channels/davis-video-themes.json';
  const davisCatalog = '/content/channels/williamdavismd.json';
  const videosHub = '/videos/?channel=davis';

  const themesDoc = categorizeDavisVideos();
  if (opts.writeThemesJson === true) {
    writeThemesJson(themesDoc);
  }
  const catalogMd = buildDavisCatalogMarkdown({ doc: themesDoc });
  const catalogEn = buildDavisCatalogSummaryI18n('en', themesDoc);
  const catalogEs = buildDavisCatalogSummaryI18n('es', themesDoc);

  const body = `## Escopo

Inspeção editorial e documental de **William Davis** — cardiologista preventivo (prática pública associada a Milwaukee, EUA) e autor de [*Barriga de Trigo* / *Wheat Belly*](${livro}) (2011). O recorte BudGanja cobre a **pessoa e o método** de divulgação clínica-popular sobre trigo (elo principal no livro, série Artes) e o **catálogo temático completo** do canal YouTube [@williamdavismd](${yt}) — **${themesDoc.categorizedCount}** vídeos.

> **Nota metodológica:** auditoria independente. Fontes: [site do autor](${site}), [Wikipedia · Wheat Belly](${wikiBook}), canal [William Davis, MD](${yt}). Sem afiliação. Distinto do [Legado](${legado}) UNIFESP/CEBRID e da [Divulgação Lair](${lair}). A ficha do livro inspecciona a **tese**; aqui inspecciona-se o **ofício** do autor-médico e o acervo de vídeos. **Não é aconselhamento médico.** **Categorização (2026-08-02):** todos os vídeos em temas editoriais → [\`davis-video-themes.json\`](${davisThemes}) e secção *Catálogo temático* abaixo.
...
| Canal YouTube | [@williamdavismd](${yt}) · [${themesDoc.categorizedCount} vídeos categorizados](${videosHub}) |
| Catálogo temático | [\`davis-video-themes.json\`](${davisThemes}) · bruto [\`williamdavismd.json\`](${davisCatalog}) |
| Data da inspeção | ${inspected} |
...
**H4:** o canal YouTube é o **arquivo vivo** do método (trigo, microbioma, coração, Undoctored) — catalogado e filtrável em [/videos/?channel=davis](${videosHub}).
...
**Aprovado na série Pessoas** — William Davis documentado com elo principal em [Barriga de Trigo](${livro}) (Artes), cruzamento [Glúten](${gluten}) e catálogo YouTube completo (${themesDoc.categorizedCount} vídeos).

[▶ Pessoas](${hub}) · [▶ Barriga de Trigo](${livro}) · [▶ Glúten](${gluten}) · [▶ Vídeos Davis](${videosHub}) · [Site do autor](${site})
...
Editorial inspection of **William Davis** — preventive cardiologist and author of [Wheat Belly](${livro}) (2011). Focus is the **person and method** of clinical-popular outreach on wheat, plus the full thematic catalog of his YouTube channel [@williamdavismd](${yt}) (**${themesDoc.categorizedCount}** videos). Book sheet: [Wheat Belly / Barriga de Trigo](${livro}).

> **Method note:** independent audit from [author site](${site}) / [Wikipedia · Wheat Belly](${wikiBook}) / [YouTube](${yt}). Distinct from UNIFESP Legacy and Lair outreach. **Not medical advice.** Indexing ≠ endorsement.
...
| Harmful-products link | [Gluten](${gluten}) |
| YouTube | [@williamdavismd](${yt}) · [${themesDoc.categorizedCount} videos](${videosHub}) |
| Date | ${inspected} |
...
| Video archive | Full channel themed in [/videos/?channel=davis](${videosHub}) |

${catalogEn}

## Status

**Approved in People** — primary Arts link [Wheat Belly](${livro}); video catalog categorized.
`;

  const contentEs = `## Alcance

Inspección editorial de **William Davis** — cardiólogo preventivo y autor de [Barriga de Trigo](${livro}) (2011). El recorte es la **persona y el método** de divulgación clínica-popular sobre el trigo, más el catálogo temático completo del canal [@williamdavismd](${yt}) (**${themesDoc.categorizedCount}** vídeos). Ficha del libro: [Barriga de Trigo](${livro}).

> **Nota metodológica:** auditoría independiente ([sitio del autor](${site}) / [YouTube](${yt})). Distinto del Legado UNIFESP y de Lair. **No es consejo médico.** Indexar ≠ respaldar.
...
| Productos nocivos | [Gluten](${gluten}) |
| YouTube | [@williamdavismd](${yt}) · [${themesDoc.categorizedCount} vídeos](${videosHub}) |
| Fecha | ${inspected} |
...
**Aprobado en Personas** — vínculo principal [Barriga de Trigo](${livro}); catálogo de vídeos categorizado.
`;

  return { body, contentEn, contentEs, wiki: site, themesDoc };
}

function buildWilliamDavisPost(opts) {
  const { body, contentEn, contentEs, wiki } = buildWilliamDavisBodies(opts);
  return figuraPost({
    title:
