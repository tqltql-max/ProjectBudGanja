...
async function handleApi(context) {
  ...
  if (apiPath === '/api/etymology' && method === 'GET') {
    try {
      const etymologyRoute = require('./api-routes/etymology.js');
      const routed = await etymologyRoute.match({
        url: apiPath,
        method: method,
        req: { query: url.search.startsWith('?') ? url.search.slice(1) : url.search },
        headers: headersFromRequest(request)
      });
      if (routed) return toPagesResponse(routed);
    } catch (e) {
