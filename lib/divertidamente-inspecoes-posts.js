...
**H1:** a mensagem que o laboratório guarda com raciocínio: **nenhuma emoção é lixo** — inclusive a Tristeza, que permite pedido de ajuda e ligação.  
...
| [Cruzamento Raiva × Venom × Vida](${cruzamento}) | Mapa: sala de comando + «nós» + ficar |
...
**Aprovado na série Artes** — *Divertida Mente* (2015) documentado com raciocínio: génese Pixar, mensagem de que **todas as emoções importam**, e rede completa com [emoção](${EMOCAO}) e as cinco palavras da Riley.
...
function buildDivertidamentePost() {
  ...
  return artePost({
    ...
    excerpt:
      'Artes: Divertida Mente (Pixar, 2015) — literacia emocional com raciocínio; Alegria, Tristeza, Raiva, Medo e Nojinho em rede com a série Palavras. Todas as emoções importam.',
    excerptEn:
...
function buildNojinhoPost() {
  return buildRileyEmotionPost({
    ...
    filmNote:
      'Nojinho é o nome que o Brasil memorizou — raciocínio preciso. Protege Riley do que repele (sim, brócolis). Por baixo do gag há literacia: **aversão também cuida**. A ficha honra a forma do filme e aponta o substantivo *nojo* como irmão lexical adulto.',
