...
/** Vídeos âncora extra (lives / busca) fundidos no hub /videos e na categorização. */
const EXTRA_VIDEOS = [
  {
    id: 'S61hC_9hmf8',
...
function categorizeLairVideos(opts) {
  ...
  return {
    ...
    method:
      'Classificação exclusiva por palavras-chave no título (+ hits de lair-sugar-hits.json forçam eixo açúcar/diabetes). Âncoras extra (lives) entram no hub /videos e nesta categorização.',
...
> **Nota:** indexar ≠ endossar. A categorização organiza o acervo para pesquisa editorial; **não** valida claims clínicos. O acervo entra na aba [/videos/](/videos/?channel=lair); âncoras extra (ex.: live cannabis \`${(doc.extraAnchors || []).join('`, `')}\`) também.
