...
Inspeção editorial do canal **[${name}](${channelUrl})** (${handle}) — arquivo público de **fauna, expedições e ofício selvagem** desde **2016**. Este arquivo **merece [respeito](${respeito})**: ${count} vídeos classificados, não um dump nem um recorte de polémica. No laboratório BudGanja entra no hub **[Vídeos](${videosHub})** como canal de natureza — distinto da ficha de **pessoa / legado** [${pessoa}](${pessoa}). O handle leva [selvagem](${selvagem}): a palavra do bosque; o ecrã é o arquivo.
...
| Elo Palavras | [respeito](${respeito}) · [selvagem](${selvagem}) · [especial](${especial}) · [animal](${animal}) · [risco](${risco}) · [verdade](${verdade}) |
...
**H4:** [selvagem](${selvagem}) pede **[respeito](${respeito}) sem romantizar dano** — esta ficha indexa o arquivo; os limites públicos (multas, controvérsias) ficam na ficha de pessoa.  
**H5:** fecho = [Valeu !!!](${mantra}) — o melhor recorte possível *deste* arquivo *neste* laboratório.
...
**Aprovado** — canal **${name}** documentado como hub de **natureza / fauna** no BudGanja; ${count} vídeos classificados pelos temas de título; âncora «${mdTitle(seed.title)}». Distinto da ficha de [pessoa](${pessoa}). Arquivo com [respeito](${respeito}) de ofício. Indexar ≠ endosso.

[▶ Vídeos · Rasmussen](${videosHub}) · [▶ Pessoa](${pessoa}) · [▶ Selvagem](${selvagem}) · [▶ Respeito](${respeito}) · [▶ Animais](${animais}) · [▶ Canais](${hub}) · [▶ Valeu !!!](${mantra})
`;
...
function buildRasmussenCanalPost(seriesOrder) {
  ...
  return artePost({
    ...
    excerpt:
      'Canais: Richard Rasmussen (@RichardRasmussenSelvagem) — arquivo YouTube de fauna e expedições desde 2016; respeito de arquivo; hub /videos; distinto da ficha de pessoa.',
    excerptEn:
