...
function renderLojaMaterialsHtml(projectId, compact) {
  ...
  const title = compact ? 'Materiais recomendados' : 'Equipamentos verificados';

  return (
    '<aside class="equip-loja-materials highlight' +
    (compact ? ' equip-loja-materials--compact' : '') +
    '">' +
    '<p class="equip-loja-eyebrow">' +
...
function buildI18nNoteHtml(post) {
  ...
  if (!en && !es) return '';
  let html = '<aside class="post-i18n-note" data-post-i18n-note hidden>';
  if (en) {
    html +=
      '<p class="post-i18n-locale" data-i18n-locale="en" lang="en" hidden>' +
...
function appendPostExtras(post, bodyHtml) {
  ...
  // Blocos "loja parceira" desactivados — patrocínios entram organicamente nas pesquisas.
  return html;
}
