  ...
  return {
    ...
    async getUsers() {
      const result = await db.execute(
        'SELECT id, email, name, picture, provider, profile_json, is_admin, created_at, updated_at FROM users'
      );
      const users = {};
    ...
    async enqueueEmail(job) {
      return insertEmailJob(db, job);
    },
    async findEmailByIdempotencyKey(key) {
      return findEmailByIdempotencyKey(db, key);
    },
    async listPendingEmails(limit) {
      return listPendingEmails(db, limit);
    },
    async listRecentEmails(limit, status) {
      return listRecentEmails(db, limit, status);
    },
    async markEmailSent(id) {
      return markEmailSent(db, id);
    },
    async markEmailFailed(id, error, attempts) {
      return markEmailFailed(db, id, error, attempts);
    },
    async getEmailQueueStats() {
      return getEmailQueueStats(db);
    }
  };
}
