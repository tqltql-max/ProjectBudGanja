...
function resolveStoreBackend(options) {
  ...
  if (options.netlify || options.cloudflare) {
    return hasRemoteDatabase() ? 'sql' : (options.cloudflare ? 'sql' : 'blobs');
  }
...
function loadR2Store() {
  return require('./store-r2.js').createR2UploadStore;
}

function loadNoopUploads() {
  return require('./store-r2.js').createNoopUploadStore;
}
...
async function createAppStore(options) {
  ...
  const backend = resolveStoreBackend({ netlify, cloudflare });
  ...
  try {
    ...
    if (cloudflare) {
      const uploads = options.r2Uploads
        ? loadR2Store()(options.r2Uploads)
        : loadNoopUploads()();
