...
Inspeção editorial da palavra **[ação](${self})** — o **fazer nomeado** (lat. *āctiō* ← *agere*). Pedido de campo: *inspeção da palavra ação*. Esta ficha cobre o **objecto**, a grafia **ação** (BR / AO90) × **acção** (PT tradicional), os **sentidos** (ofício · processo · bolsa · filme · física) e o corte com o **[gesto](${gesto})**. Fecho: [Valeu !!!](${mantra}).
...
| Étimo (trabalho) | Lat. *āctiō, -ōnis* ← *agere / āctus* («fazer, impulsionar») — confiança: **alta** |
...
| Grafia **ação** | BR e AO90 — o *ç* absorve o *ct* na fala | Alta (uso vivo) |
| Ing. *action* | Cognato, não étimo do português | Alta (paralelo) |

**Veredicto etimológico:** origem **latina** fechada (*agere* → *āctiō* → ação / acção). O que oscila é a **sala de uso**, não o étimo.
...
[▶ Palavras](${hub}) · [▶ Gesto](${gesto}) · [▶ Caminho](${caminho}) · [▶ Meter marcha](${marcha}) · [▶ Valeu !!!](${mantra}) · [▶ Vida](${vida})
`;
...
function buildAcaoPost() {
  ...
  return makePalavra({
    ...
    excerpt:
      'Palavras: «ação» / acção (lat. āctiō ← agere) — ofício; gesto ≠ ação ≠ ações da bolsa; Valeu !!!',
    excerptEn:
      'Words: “ação” / acção (Lat. āctiō ← agere) — craft; gesture ≠ action ≠ stock shares; Valeu !!!',
    excerptEs:
      'Palabras: «ação» / acção (lat. āctiō ← agere) — oficio; gesto ≠ acción ≠ acciones de bolsa; ¡Valeu !!!',
