...
function plantHealthNotesMarkdown(plant, loc) {
  if (plant.slug === 'abacate') return abacatePlantNotesMarkdown(loc);
...
function abacatePlantNotesMarkdown(loc) {
  const derivados = '/posts/post-inspecao-derivado-abacate.html';
...
## Indigenous name and special meaning

The Portuguese word **abacate** comes from Nahuatl **āhuacatl** (via Spanish *aguacate*) — the language of indigenous Mesoamerican peoples (Mexico / Central America). In Nahuatl the same word names the **fruit** and also **testicle**, by shape and how fruits hang on the tree; that is the cultural charge of fertility/vigor often cited in ethnobotany. It is **not** a Tupi word. In the Andes the fruit is often called **palta** (Quechua / Palta people) — another indigenous naming tradition.

Source of departure: [Wikipedia · Abacate](${wiki}).

## Chemistry of the fresh fruit (educational)

Approximate pulp profile (varies by cultivar — Hass, Fuerte, etc.):

| Class | Markers / notes |
|-------|-----------------|
| Lipids | Mostly **monounsaturated** fat (oleic acid dominant); also palmitic and linoleic |
| Carbohydrates | Relatively low free sugars vs many fruits; some fiber |
| Micronutrients | Potassium, folate, vitamin E, carotenoids (e.g. lutein) |
| Phenolics | Polyphenols; pulp browns via **polyphenol oxidase (PPO)** when cut |
| Defense toxin | **Persin** — antifungal compound concentrated in leaves/seed/skin; toxic to many pets |

**BudGanja framing:** ripe pulp as whole food is the lab’s low-risk baseline. Leaves/seed are **not** kitchen ingredients. Industrial products that add sugar and additives are audited in [Avocado derivatives](${derivados}) (link also to [sugarcane / free sugars](${cana})).

## Related risk-derivatives sheet

[Avocado derivatives — sugar, additives and industrial chemistry](${derivados}).
`;
  }
...
## Nombre indígena y sentido especial

La palabra portuguesa **abacate** / española **aguacate** viene del náhuatl **āhuacatl** — lengua de pueblos indígenas mesoamericanos. En náhuatl nombra el **fruto** y también **testículo**, por la forma; carga cultural de fertilidad/vigor. **No** es una palabra tupí. En los Andes suele llamarse **palta**.

Fuente: [Wikipedia · Abacate](${wiki}).

## Química del fruto fresco (educativa)

| Clase | Marcadores / notas |
|-------|--------------------|
| Lípidos | Sobre todo grasa **monoinsaturada** (ácido oleico); también palmítico y linoleico |
| Carbohidratos | Azúcares libres relativamente bajos vs muchas frutas; fibra |
| Micronutrientes | Potasio, folato, vitamina E, carotenoides (p. ej. luteína) |
| Fenólicos | Polifenoles; pardeamiento por **polifenol oxidasa (PPO)** |
| Toxina de defensa | **Persina** — concentrada en hojas/hueso/cáscara; tóxica para muchas mascotas |

**Encuadre BudGanja:** la pulpa madura como alimento entero es la línea de bajo riesgo. Hojas/hueso **no** son ingredientes de cocina. Productos industriales con azúcar y aditivos: [Derivados del aguacate](${derivados}) (ver también [caña / azúcares libres](${cana})).

## Ficha relacionada — Derivados de riesgo

[Derivados del aguacate — azúcar, aditivos y química industrial](${derivados}).
`;
  }

  return `

## Nome indígena e sentido especial

A palavra portuguesa **abacate** vem do náuatle **āhuacatl** (via espanhol *aguacate*) — língua de povos indígenas mesoamericanos (México / América Central). No náuatle o mesmo vocábulo nomeia o **fruto** e também **testículo**, pela forma e pelo modo como os frutos pendem na árvore; daí a carga cultural de fertilidade / vigor frequentemente citada na etnobotânica. **Não** é palavra tupi. Nos Andes o fruto chama-se muitas vezes **palta** (tradição quéchua / povo Palta) — outro nome indígena, outra região.

Fonte de partida: [Wikipédia · Abacate](${wiki}).

## Química do fruto fresco (educacional)

Perfil aproximado da polpa (varia por cultivar — Hass, Fuerte, etc.):

| Classe | Marcadores / notas |
|--------|-------------------|
| Lipídios | Predominam gorduras **monoinsaturadas** (ácido oleico); também palmítico e linoleico |
| Hidratos de carbono | Açúcares livres relativamente baixos face a muitas frutas; fibra presente |
| Micronutrientes | Potássio, folato, vitamina E, carotenóides (ex.: luteína) |
| Fenólicos | Polifenóis; o escurecimento da polpa cortada deve-se à **polifenol oxidase (PPO)** |
| Toxina de defesa | **Persina** — composto antifúngico concentrado em folhas / caroço / casca; tóxico para muitos animais domésticos |

**Enquadramento BudGanja:** a polpa madura como alimento inteiro é a linha de baixo risco do laboratório. Folhas e caroço **não** são ingredientes de cozinha. Produtos industriais que adicionam açúcar e aditivos são auditados em [Derivados do abacate](${derivados}) (cruzar também com [cana / açúcares livres](${cana})).

## Ficha relacionada — Derivados de risco

[Derivados do abacate — açúcar, aditivos e química industrial](${derivados}).
`;
}
...
function buildPlantaInspecaoPost(plant, seriesOrder) {
  ...
  return {
    ...
    excerpt: isCurcuma
      ...
      : isAbacate
        ? 'Abacate (*Persea americana*): āhuacatl (náuatle), química da polpa (lípidos, persina, PPO) e elo com a ficha de derivados industriais açucarados/aditivados.'
    ...
    excerptEn: isCurcuma
      ...
      : isAbacate
        ? 'Avocado (*Persea americana*): āhuacatl (Nahuatl), pulp chemistry (lipids, persin, PPO) and link to the industrial sugary/additive derivatives sheet.'
    ...
    excerptEs: isCurcuma
      ...
      : isAbacate
        ? 'Aguacate (*Persea americana*): āhuacatl (náhuatl), química de la pulpa (lípidos, persina, PPO) y vínculo con la ficha de derivados industriales azucarados/aditivados.'
