...
function plantInspectedDate(plant) {
  const d = plant && plant.inspecaoDate ? String(plant.inspecaoDate).trim() : '';
  return /^\d{4}-\d{2}-\d{2}$/.test(d) ? d : '2026-07-31';
}
...
function buildBody(plant, loc) {
  ...
  const inspected = plantInspectedDate(plant);
...
function buildPlantaInspecaoPost(plant, seriesOrder) {
  ...
  return {
    ...
    date: plantInspectedDate(plant) + 'T14:00:00.000Z',
