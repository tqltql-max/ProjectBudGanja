...
/** Ordem de exibição no hub: MovReCam → CANABinALL → Inspetor → Lair. */
const CHANNEL_META = [
  {
    id: 'movrecam',
    label: 'MovReCam',
...
function seriesFor(channelId, v, lairThemeById) {
  const tags = [];
  ...
  if (channelId === 'lair' && lairThemeById) {
    const themeId = lairThemeById.get(String((v && v.id) || ''));
 ...
 * @param {Map<string, string>} [lairThemeById]
 */
function topicsFor(channelId, v, overrides, lairThemeById) {
  ...
  if (channelId === 'lair' && lairThemeById) {
    const themeId = lairThemeById.get(id);
...
function normalizeVideo(v, channelId, overrides, lairThemeById) {
  ...
  return {
    ...
    channel: channelId,
    series: seriesFor(channelId, v, lairThemeById),
    topics: topicsFor(channelId, v, overrides, lairThemeById)
  };
}
...
function loadCatalogChannel(root, meta, overrides, lairThemeById) {
  ...
  return {
    ...
    videos: sortNewestFirst(
      (raw.videos || [])
        .map((v) => normalizeVideo(v, meta.id, overrides, lairThemeById))
        .filter(Boolean)
    )
  };
}

function loadLairChannel(root, meta, overrides, lairThemeById) {
  const raw = readJson(path.join(root, meta.catalogRel), null) || {
    videos: [],
    ...
    const loaded =
      meta.source === 'youtube-feed'
        ...
        : meta.source === 'lair-catalog'
          ? loadLairChannel(root, meta, overrides, lairThemeById)
          : loadCatalogChannel(root, meta, overrides, lairThemeById);
