...
| Mãos | **mãos a obra** (plural, sem crase) — **levante** o que já está; **insira** o novo — lapso de campo *insiera* |
...
| Família | *upsert* · *opsert* · *opsertar* · *Opsert.* · *mãos a obra* · *upsertar* (calco) · SQL *MERGE* |
...
**H8:** o grito é **mãos a obra** (plural, sem crase). O acento da norma (*mãos à obra*) **dificulta** a boca — fica no mapa, não na fala.

Passos: blend EN → nome nosso (**opsert**) → mãos a obra → mapa SQL → gesto lab (slug) → limites → [Valeu !!!](${mantra}).
...
| **Mãos a obra** | Grito plural, sem crase (*insiera* no ouvido) | *Mãos à obra* (norma) — o acento dificulta |
...
3. Opsertar **uma** ficha — não duas com o mesmo nome. **Mãos a obra.**  
...
| **Mãos** | «Mãos a obra» · «levante e insira» (*insiera*) | Bom: plural, sem crase · Mau: crase que trava a boca |
...
**Aprovado na série Palavras** — *upsert* fichado como **portmanteau de identidade**; nome nosso **opsert** / **Opsert.**; grito **mãos a obra** (plural, sem crase); ramos *levante e insira*; calco *upsertar* no mapa, não na fala; ≠ *replace* ≠ [commitar](${commitar}); o lab já opserta no *slug*; fecho [Valeu !!!](${mantra}).
...
Editorial inspection of **[upsert](${self})** — English portmanteau of *update* + *insert*. Lab living name: **opsert** (verb **opsertar**; close **Opsert.**). Craft cry: **mãos a obra** (plural, no crase — the accent gets in the way; *mãos à obra* stays on the map). Two branches of the same gesture: **raise** what is already there; **insert** what is not. House files stay \`upsert-*.js\`. Close: [Valeu !!!](${mantra}).
...
| Hands | **mãos a obra** (plural, no crase) — raise and insert |
...
**Lab:** \`upsertPost\` looks up the *slug* — update if present, insert if not — then [commitar](${commitar}) the trace. Cry: **Mãos a obra.**
...
Inspección de **[upsert](${self})** — portmanteau inglés de *update* + *insert*. Nombre vivo del lab: **opsert** (verbo **opsertar**; cierre **Opsert.**). Grito: **mãos a obra** (plural, sin crasis — el acento estorba; *mãos à obra* queda en el mapa). Dos ramas del mismo gesto: **levantar** lo que ya está; **insertar** lo que no. Los ficheros siguen \`upsert-*.js\`. Cierre: [¡Valeu !!!](${mantra}).
...
| Manos | **mãos a obra** (plural, sin crasis) — levantar e insertar |
...
**Lab:** \`upsertPost\` busca el *slug* — actualiza si está, inserta si no — luego [commitar](${commitar}) el rastro. Grito: **Mãos a obra.**
...
function buildUpsertPost() {
  ...
  return makePalavra({
    ...
    excerpt:
      'Palavras: opsert (boca BR de upsert) — mãos a obra (plural, sem crase); chave/slug; MERGE ≠ replace; calco upsertar; elo commitar; Valeu !!!',
    excerptEn:
      'Words: opsert (BR mouth of upsert) — mãos a obra (plural, sem crase); key/slug; MERGE ≠ replace; calque upsertar; link commitar; Valeu !!!',
    excerptEs:
      'Palabras: opsert (boca BR de upsert) — mãos a obra (plural, sem crase); clave/slug; MERGE ≠ replace; calco upsertar; vínculo commitar; ¡Valeu !!!',
