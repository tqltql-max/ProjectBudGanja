...
function buildVentilacaoTendaPost() {
  return verificacaoPost({
    ...
    titleEn: 'Inspection: tent ventilation system',
    titleEs: 'Inspección: sistema de ventilación de la carpa',
    excerpt:
      'Verificação de fluxo de ar, ruído e trocas térmicas no duto de exaustão e filtro de carvão do laboratório.',
    excerptEn:
      'Airflow, noise and heat-exchange check on the lab exhaust duct and carbon filter.',
    excerptEs:
      'Verificación de flujo de aire, ruido e intercambio térmico en el ducto de extracción y filtro de carbón del laboratorio.',
...
function buildMarsHydroInspectionPost() {
  return verificacaoPost({
    ...
    titleEn: 'Inspection: Mars Hydro Brasil equipment',
    titleEs: 'Inspección: equipos Mars Hydro Brasil',
    excerpt:
...
- **Método:** (1) levantamento do catálogo BR em 2026-06-28; (2) agrupamento por família de produto; (3) validação de cobertura LED × volume da tenda; (4) cruzamento com [inspeções de ambiente indoor](/posts/post-guia-calculadora-luximetro.html), [ventilação](/posts/post-inspecao-ventilacao-tenda.html) e [pesquisa em unidade Mars Hydro](/posts/post-otimizacao-propagacao-vegetal.html).
...
3. **Combos LED + tenda** pré-dimensionados (70×70, 100×100, 120×60 cm) simplificam primeira compra; validar DLI com fotoperíodo real no [diário de pesquisas](/cultivo/).
...
## Origem da palavra e controle de incêndio

- **Palavra:** [Mars Hydro](/posts/post-inspecao-palavra-mars-hydro.html) — *Mars* (planeta / deus) + *Hydro* (água); rasto de campo *marshydrobr*.
- **Mapa:** [Objectos perigosos para controle de incêndio](/posts/post-inspecao-palavra-objetos-perigosos-incendio.html) — LED, [fonte](/posts/post-inspecao-palavra-fonte.html), [tenda](/posts/post-inspecao-palavra-tenda.html), [extensão](/posts/post-inspecao-palavra-extensao.html) e [exaustor](/posts/post-inspecao-palavra-exaustor.html) são **objectos**, não o logótipo.
- **Irmã de corredor:** [Vivosun](/posts/post-inspecao-vivosun.html) (revenda BR) · [palavra Vivosun](/posts/post-inspecao-palavra-vivosun.html).
- **Ficha ≠ laudo eléctrico** nem NR-23.

## Complementaridade com o Inspetor BudGanja

| Necessidade | Ferramenta / registo BudGanja |
|-------------|------------------------------|
| PPFD / DLI alvo | [Calculadora Luxímetro](/calculadoras/luximetro.html) · [Inspeção: Cultivo Indoor](/posts/post-guia-calculadora-luximetro.html) |
| VPD e clima | [Calculadora VPD](/calculadoras/cultivo-lab.html?mode=vpd) |
...
| Registo de ciclos | [Diário de Pesquisas](/cultivo/) |
...
const { buildCelularRiscosCriancasInspecaoPost } = require('./celular-riscos-criancas-inspecao-post.js');
const { buildPatineteEletricoCriancasInspecaoPost } = require('./patinete-eletrico-criancas-inspecao-post.js');
const { buildVivosunVerificacaoPost } = require('./vivosun-verificacao-inspecao-post.js');

const EQUIPAMENTO_VERIFICACAO_POSTS = [
