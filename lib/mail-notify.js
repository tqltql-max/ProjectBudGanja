...
async function notifyInspectorNewAlertSubscriber({ name, email }) {
  const inspectorEmail =
    String(process.env.NOTIFY_EMAIL || process.env.CONTACT_EMAIL || 'tql.tql@gmail.com').trim();
...
async function sendPasswordResetEmail(opts) {
  ...
  const resetUrl = String(opts && opts.resetUrl || '').trim();
  if (!to || !resetUrl) return { sent: false, reason: 'missing_payload' };
  const siteUrl = getSiteUrl();
  const subject = '[BudGanja] Recuperação de senha';
  const text =
    'Recebemos um pedido para redefinir sua senha no Inspetor BudGanja.\n\n' +
    'Abra este link para criar uma nova senha (válido por 1 hora):\n' + resetUrl + '\n\n' +
    'Se você não pediu essa alteração, ignore este e-mail.\n\n' +
    '— Inspetor BudGanja\n' + siteUrl + '\n';
  return sendMail({ to: to, subject: subject, text: text });
}

async function notifyInspectorNewGtarpStreamer(row) {
  const inspectorEmail =
    String(process.env.NOTIFY_EMAIL || process.env.CONTACT_EMAIL || 'tql.tql@gmail.com').trim();
  const subject = '[BudGanja] Candidatura GTA RP: ' + (row.characterName || 'streamer');
  const text =
    'Nova candidatura de streamer para a página GTA RP:\n\n' +
    'Personagem: ' + (row.characterName || '—') + '\n' +
    'Função: ' + (row.role || '—') + '\n' +
    'YouTube: ' + (row.youtubeUrl || '—') + '\n' +
    'Kick: ' + (row.kickUrl || '—') + '\n' +
    'Twitch: ' + (row.twitchUrl || '—') + '\n' +
    'Contacto: ' + (row.contactName || '—') + ' <' + (row.contactEmail || '') + '>\n' +
    'Notas: ' + (row.notes || '—') + '\n' +
    'Data: ' + (row.submittedAt || new Date().toISOString()) + '\n\n' +
    'Rever: ' + getSiteUrl() + '/gtarp-admin.html\n';
  console.log('[notify]', subject, '-', row.contactEmail);
  return sendMail({ to: inspectorEmail, subject, text });
}
...
module.exports = {
  ...
  notifyInspectorNewGtarpStreamer,
