...
Inspeção editorial e documental do **legado público** de **Richard Rasmussen** (São Paulo, 15 de fevereiro de 1970) — naturólogo, economista, **biólogo** (CRBio 068861/01-D) e apresentador brasileiro. O recorte não é biografia televisiva completa: é registar, com fontes públicas e com o **mérito que lhe é devido**, o ofício de **levar fauna ao ecrã** — e os **limites** que o laboratório não esconde. O handle do canal é [Selvagem](${selvagem}): a palavra do bosque; esta ficha é a **pessoa**.

> **Nota metodológica:** auditoria independente do Inspetor BudGanja. Fontes: [Wikipédia · Richard Rasmussen](${wiki}), [site](${site}), [canal YouTube](${yt}). **Sem afiliação** com Richard Rasmussen, emissoras, Embratur ou partidos. Capítulo de **Legado** cultural / naturalista — distinto do eixo clínico Ticão–Carlini. **Pessoa ≠ canal.** Indexar ≠ endosso político. Esta inspeção é **[especial](${especial})**: pessoa + arquivo.
...
| Elo BudGanja | Legado Cap. 10 · [selvagem](${selvagem}) · [animal](${animal}) · hub [Animais](${animais}) |
| Data da inspeção | ${inspected} |

## Por que esta inspeção existe

O laboratório já fichou [selvagem](${selvagem}) — *silva*, natureza, **sem romantizar dano**. Rasmussen é a voz BR mais reconhecível desse ecrã: serpentes, biomas, expedição. Omiti-lo seria falhar o método — **crédito a quem merece** — no elo entre Palavras, Animais e Canais. Colocá-lo no Legado junto de exploradores como [Amyr](${amyr}) e [Tamara](${tamara}) é continuidade de ofício no território, não fusão de pessoas.
...
- **H3:** [selvagem](${selvagem}) no handle obriga o lab a aplicar a própria ficha: respeito à natureza **e** [risco](${risco}) / [verdade](${verdade}) quando o recorde público pede.
- **H4:** limites (criadouro, multas, teses controversas) **não apagam** o ofício de ecrã — e o ofício **não apaga** os limites.
- **Método:** (1) síntese biográfica pública; (2) carreira de ecrã; (3) mérito; (4) limites; (5) elos; (6) status.
...
1. **Ofício de ecrã** — duas décadas a traduzir fauna e biomas para um público amplo, com CRBio à vista.
2. **Arquivo YouTube** — o canal próprio (2016) torna o ofício **inspeccionável** no lab: ver [ficha do canal](${canal}) (${videos}).
3. **Literacia de espécie** — serpente, felino, rio, bioma: o título costuma **nomear** o ser, não só o hype.
4. **Elo lexical** — [selvagem](${selvagem}) no handle; [animal](${animal}) e [Animais](${animais}) no hub do lab.
5. **Complementaridade exploratória** — no Legado, vizinho de [Amyr](${amyr}) / [Tamara](${tamara}): território e partida, não o mesmo mar.

## Limites (para não virar hagiografia)

- **Criadouro (2005):** o IBAMA fechou a «Casa da Tartaruga» após autuações (fugas, mortes, origem de animais) — facto público; o lab **não** rejulga o processo.
- **Multas ambientais:** reportagem à data da nomeação Embratur (2019) e acordo posterior de reparação em unidades de conservação (2020) — [verdade](${verdade}) de recorde, não pamphlet.
...
| [risco](${risco}) · [verdade](${verdade}) · [respeito](${respeito}) | Mapa honesto do ofício e dos limites |
...
| Camada | Ligação |
|--------|---------|
| Mantra | [Faça o melhor!](${mantra}) — o melhor ecrã possível **com** o ser vivo, **sem** apagar [risco](${risco}) |
| Anti-armadilha | «selvagem na TV = natureza salva» **ou** «todo apresentador de fauna é fraude» = mapas incompletos |
| Par vivo | [Canal](${canal}) · [selvagem](${selvagem}) · [Animais](${animais}) |

**Veredicto:** Rasmussen fez o melhor **também ao arriscar o ecrã** — e o laboratório faz o melhor ao **creditar o ofício e declarar os limites**.

## Status

**Aprovado com mérito de ofício e limites declarados** — Richard Rasmussen · Legado Cap. 10; canal em [Canais](${canal}). Sem afiliação. Sem voto.

[▶ Canal YouTube](${canal}) · [▶ Vídeos](${videos}) · [▶ Selvagem](${selvagem}) · [▶ Legado](${legado}) · [▶ Faça o melhor!](${mantra})
`;

  const contentEn = `## Scope

Editorial inspection of **Richard Rasmussen** (b. 15 Feb 1970, São Paulo) — biologist (CRBio 068861/01-D) and Brazilian wildlife presenter. Focus: two decades bringing fauna to Brazilian screens, plus the YouTube archive since 2016. Handle: [selvagem](${selvagem}) (“wild”). **Person ≠ channel.**

> Independent audit. Sources: [Wikipedia](${wiki}), [site](${site}), [YouTube](${yt}). No affiliation. Public limits (IBAMA fines, controversies) are **on the record** — not a pamphlet, not a hagiography.
...
**Approved with craft merit and declared limits** — Legacy Cap. 10. Channel catalogued separately.

[▶ Channel](${canal}) · [▶ Videos](${videos}) · [▶ Selvagem](${selvagem})
`;

  const contentEs = `## Alcance

Inspección de **Richard Rasmussen** (n. 15 feb 1970, São Paulo) — biólogo (CRBio 068861/01-D) y presentador de fauna brasileño. Foco: dos décadas de naturaleza en pantalla y el archivo YouTube desde 2016. Handle: [selvagem](${selvagem}). **Persona ≠ canal.**

> Auditoría independiente. Fuentes: [Wikipedia](${wiki}), [sitio](${site}), [YouTube](${yt}). Sin afiliación. Límites públicos (multas IBAMA, controversias) quedan **en el registro**.

## Estado

**Aprobado con mérito de oficio y límites declarados** — Legado Cap. 10. Canal aparte.

[▶ Canal](${canal}) · [▶ Vídeos](${videos}) · [▶ Selvagem](${selvagem})
`;
...
function buildRasmussenPessoaPost(seriesOrder) {
  ...
  return pessoaPost({
    title: 'Inspeção: Richard Rasmussen — ofício selvagem no ecrã brasileiro',
    titleEn: 'Inspection: Richard Rasmussen — wild craft on Brazilian screens',
    titleEs: 'Inspección: Richard Rasmussen — oficio salvaje en la pantalla brasileña',
    excerpt:
      'Legado: Richard Rasmussen — biólogo (CRBio) e apresentador de fauna; YouTube @RichardRasmussenSelvagem; mérito de ofício e limites públicos; pessoa ≠ canal.',
    excerptEn:
      'Legacy: Richard Rasmussen — biologist (CRBio) and wildlife presenter; YouTube @RichardRasmussenSelvagem; craft merit and public limits; person ≠ channel.',
    excerptEs:
      'Legado: Richard Rasmussen — biólogo (CRBio) y presentador de fauna; YouTube @RichardRasmussenSelvagem; mérito de oficio y límites públicos; persona ≠ canal.',
