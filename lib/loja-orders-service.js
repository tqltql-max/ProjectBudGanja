...
async function dispatchOrderEmails(store, entry) {
  ...
  if (!getSmtpCredentials().configured) {
    return { inspector: notifyInspector, customer: notifyCustomer };
  }

  try {
    await enqueueEmail(store, {
      template: 'loja_order_inspector',
      to: INSPECTOR_EMAIL(),
      toName: 'Inspetor BudGanja',
      vars: { order: entry },
      idempotencyKey: 'loja-inspector:' + entry.id
    });
    await enqueueEmail(store, {
      ...
      toName: entry.nome,
      vars: { order: entry },
      idempotencyKey: 'loja-customer:' + entry.id
    });
    const processed = await processEmailQueue(store, { limit: 5, delayMs: 250 });
    notifyInspector = { sent: processed.sent >= 1, queued: true };
    notifyCustomer = { sent: processed.sent >= 2 || processed.sent >= 1, queued: true };
  } catch (e) {
    console.error('[loja-order] notify failed:', e.message);
    notifyInspector = { sent: false, error: e.message };
    notifyCustomer = { sent: false, error: e.message };
  }
...
async function createLojaOrder(store, payload, options) {
  ...
  const notify = await dispatchOrderEmails(store, entry);
