...
async function match(ctx) {
  ...
  if (url === '/api/admin/icons' && method === 'GET') {
    ...
    return jsonResponse(200, {
      ...
      icons: {
        ...
        appIcon: `/imagens/app-icon.png?v=${version}`,
        favicon: `/imagens/favicon-48.png?v=${version}`,
        faviconSvg: `/favicon.svg?v=${version}`
      }
    });
  }
  ...
  if (url === '/api/admin/update-icons' && method === 'POST') {
    ...
    // Regenerar todos os ícones (app, favicon, apple-touch, header)
    try {
    ...
    // Versiona HTML, layout.js, sw.js, version.json
    try {
    ...
    // Garante PNG/ICO no <head> de todas as páginas (igual ao npm run build)
    try {
    ...
    return jsonResponse(200, {
      ...
      icons: {
        ...
        appIcon: `/imagens/app-icon.png?v=${newVersion}`,
        favicon: `/imagens/favicon-48.png?v=${newVersion}`,
        faviconSvg: `/favicon.svg?v=${newVersion}`
      }
    });
  }
