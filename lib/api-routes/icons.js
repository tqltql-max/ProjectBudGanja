...
function runScript(scriptPath, timeoutMs) {
  ...
  return new Promise((resolve, reject) => {
    execFile(process.execPath, [scriptPath], { cwd: ROOT, timeout: ms }, (err, stdout, stderr) => {
...
function iconUrls(version) {
  ...
  return {
    ...
    icon192: `/imagens/icon-192.v${v}.png`,
    icon512: `/imagens/icon-512.v${v}.png`,
    icon512Maskable: `/imagens/icon-512-maskable.v${v}.png`,
    appleTouch: `/imagens/apple-touch-icon.v${v}.png`,
    appIcon: `/imagens/app-icon.v${v}.png`,
    favicon48: `/imagens/favicon-48.v${v}.png`,
    favicon32: `/imagens/favicon-32.v${v}.png`,
    favicon16: `/imagens/favicon-16.v${v}.png`,
    favicon: `/imagens/favicon-48.v${v}.png`,
    faviconIco: `/favicon.v${v}.ico`,
    faviconSvg: `/favicon.v${v}.svg`
  };
}

function versionedIconFiles(version) {
  const v = String(version);
  return [
    `imagens/icon-192.v${v}.png`,
    `imagens/icon-512.v${v}.png`,
    `imagens/icon-512-maskable.v${v}.png`,
    `imagens/apple-touch-icon.v${v}.png`,
    `imagens/app-icon.v${v}.png`,
    `imagens/favicon-48.v${v}.png`,
    `imagens/favicon-32.v${v}.png`,
    `imagens/favicon-16.v${v}.png`,
    `favicon.v${v}.ico`,
    `favicon.v${v}.svg`
  ];
}

function verifyGeneratedFiles(version) {
  const missing = [];
  const files = GENERATED_ICON_FILES.concat(versionedIconFiles(version || readCurrentVersion()));
...
async function match(ctx) {
  ...
  if (url === '/api/admin/update-icons' && method === 'POST') {
    ...
    // Bump primeiro: o gerador grava icon-192.v{versão}.png e o HTML aponta para essa versão.
    // Gerar e só depois incrementar deixava o site a pedir vN+1 com ficheiros ainda em vN.
    ...
    const missingAfterGenerate = verifyGeneratedFiles(newVersion);
    ...
    try {
      await runScript(path.join(ROOT, 'scripts', 'stamp-assets.js'), 300000);
    } catch (e) {
    ...
    try {
      await runScript(path.join(ROOT, 'scripts', 'sync-icon-head.js'), 300000);
    } catch (e) {
