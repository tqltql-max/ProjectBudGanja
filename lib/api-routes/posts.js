...
async function match(ctx) {
  ...
  if (url === '/api/posts' && method === 'GET') {
    ...
    if (session) {
      const list = categoryParam
        ? posts.filter((p) => (p.category || 'pesquisa') === categoryParam)
        : posts;
  ...
  if (postMatch) {
    ...
    if (method === 'GET') {
      ...
      const session = await getAdminSession(store, cookie);
      if (!session && !isPubliclyListedPost(found)) return jsonResponse(404, { error: 'post not found' });
