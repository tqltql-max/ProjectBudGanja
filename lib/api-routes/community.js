...
async function match(ctx) {
  ...
  if (url === '/api/community/plant-id' && method === 'POST') {
    const session = await getUserSession(store, cookie);
    if (!session) return jsonResponse(401, { error: 'Sessão expirada. Entre de novo para publicar.' });
    let payload;
    try {
      payload = JSON.parse(bodyRaw || '{}');
    } catch (e) {
    ...
    try {
      ...
      return jsonResponse(500, {
        error: detail
          ? ('Não foi possível publicar o pedido (' + detail + ').')
          : 'Não foi possível publicar o pedido. Tente de novo.'
      });
    }
  }
