...
**H-clique:** [Valeu !!!](${mantra}) no quarto é [ligar](${ligar}) com [risco](${risco}) visto (fio, calor, olho) — não esfregar a cúpula.
...
5. Fechar com [Valeu !!!](${mantra}) **neste** soquete, hoje.
...
|----------------|---------|
| Expressão | [Valeu !!!](${mantra}) — o melhor **neste** vaso, com clique e medida |
...
**H4:** fecho [Valeu !!!](${mantra}); ficha ≠ catálogo.
...
| [Valeu !!!](${mantra}) · [Vida](${vida}) · [Diário](${diario}) | Fecho sem esfrega |
...
**Aprovado** — **lâmpada** fichada como vaso (*lampas*); **lampada** como boca; circuito × conto separados; génio = inquilino; [gêmeos](${gemeos}) fora do étimo; fecho [Valeu !!!](${mantra}).

[▶ Palavras](${hub}) · [▶ Luz](${luz}) · [▶ Esfregar](${esfregar}) · [▶ Desejos](${desejos}) · [▶ Três](${tres}) · [▶ Valeu !!!](${mantra})
`;

  const contentEn = `## Scope

Inspection of Portuguese **lâmpada** (typed **lampada**). It is the **vessel** — in the room it holds [light](${luz}); in the tale it holds Aladdin’s genie. Two lamps, one word: **circuit** (switch, watts) vs **wish** (rub, three desires). Links: [ligar × desligar](${ligar}), [abracadabra](${abracadabra}), [genial](${genial}), [Valeu !!!](${mantra}).
...
**Lamp = vessel.** If you hear lampada, read lâmpada. Rubbing the bulb is not [turning it on](${ligar}). Close with [Valeu !!!](${mantra}) at this socket.

## Status

**Approved** — vessel sheet; circuit × tale split; genie = tenant; twins out of etymon.

[▶ Words](${hub}) · [▶ Light](${luz}) · [▶ Valeu !!!](${mantra})
`;

  const contentEs = `## Alcance

Inspección de **lâmpada** (teclado **lampada**). Es el **vaso**: en el cuarto contiene la [luz](${luz}); en el cuento, al genio de Aladino. Dos lámparas, una palabra: **circuito** × **deseo**. Vínculos: [ligar × desligar](${ligar}), [abracadabra](${abracadabra}), [genial](${genial}), [¡Valeu !!!](${mantra}).
...
**Lámpara = vaso.** Si oyes lampada, lee lâmpada. Frotar la bombilla no es [encender](${ligar}). Cerrar con [¡Valeu !!!](${mantra}).

## Estado

**Aprobada** — ficha de vaso; circuito × cuento; genio = inquilino.

[▶ Palabras](${hub}) · [▶ Luz](${luz}) · [▶ ¡Valeu !!!](${mantra})
`;
...
function buildLampadaPost() {
  ...
  const post = makePalavra({
    ...
    excerpt:
      'Palavras: «lâmpada» (lampada) — vaso de luz no quarto × lâmpada de Aladim; génio = inquilino; ≠ gêmeos; elos luz, ligar e abracadabra; Valeu !!!',
    excerptEn:
      'Words: “lâmpada” — room vessel of light × Aladdin’s lamp; genie = tenant; ≠ twins; links luz, ligar and abracadabra; Valeu !!!',
    excerptEs:
      'Palabras: «lâmpada» — vaso de luz × lámpara de Aladino; genio = inquilino; ≠ gemelos; vínculos luz, ligar y abracadabra; ¡Valeu !!!',
