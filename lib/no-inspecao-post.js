...
> **Nota metodológica:** auditoria independente. Fontes: [Wikcionário · nó](${WIKT}), lat. [nodus](${WIKT_LA}). **Ficha ≠ manual de nós náuticos, ≠ protocolo clínico, ≠ cena de força.** Elos: [corda](${corda}) (objecto) · [codorna](${codorna}) (ave — **outro** étimo; ouvido de [trocadilho](${trocadilho})) · [conexão](${conexao}) (*nectere* / nexo, vizinha). Fecho: [Valeu !!!](${mantra}).
...
| Família | *anudar* · *nódulo* · *nodo* (grafia técnica) · [node](${node}) (EN) · *desnudar* / *nuds* → [node × nuds](${node}) |
...
| **[node](${node}) / nuds** | Empréstimo EN × falso amigo *nudus* | *Node* é este étimo; *nuds/nudes* **não** — ficha irmã |

## 4. Valeu !!!

Nomear o **nó** com [verdade](${verdade}). Não fingir que a [vida](${vida}) não tem laço. O próximo gesto: **[desatar](${desatar})**. O fecho vivo: [Valeu !!!](${mantra}) — e, se o peito soltar, **[ufa](${ufa})**.

## Status

**Aprovado** — **nó** fichado: *nodus*; mora na [corda](${corda}); figura em [nó na vida!](${noVida}); solta-se em [desatar o nó](${desatarNo}).

[▶ Palavras](${hub}) · [▶ Corda](${corda}) · [▶ Desatar](${desatar}) · [▶ Nó na vida!](${noVida}) · [▶ Valeu !!!](${mantra})
`;

  const contentEn = `## Scope

Portuguese **nó** (knot) from Latin *nodus* — the crossing that holds on a [corda](${corda}) (rope). Figure: [nó na vida!](${noVida}). Craft of release: [desatar o nó](${desatarNo}). Not [codorna](${codorna}) (quail). Close: [Valeu !!!](${mantra}).

## Status

**Approved** — knot named; rope is the object; untying is the next gesture.

[▶ Words](${hub}) · [▶ Corda](${corda}) · [▶ Untie](${desatar}) · [▶ Valeu !!!](${mantra})
`;

  const contentEs = `## Alcance

**Nó** (nudo) del lat. *nodus* — el cruce que sujeta en la [corda](${corda}). Figura: [nó na vida!](${noVida}). Oficio de soltar: [desatar o nó](${desatarNo}). No es [codorna](${codorna}). Cierre: [¡Valeu !!!](${mantra}).

## Estado

**Aprobado** — nudo nombrado; la cuerda es el objeto; desatar es el gesto siguiente.

[▶ Palabras](${hub}) · [▶ Corda](${corda}) · [▶ Desatar](${desatar}) · [▶ ¡Valeu !!!](${mantra})
`;
...
function buildNoPost(seriesOrder) {
  ...
  return makePalavra({
    ...
    excerpt:
      'Palavras: «nó» (lat. *nodus*) — laço na corda e na vida; ≠ corda ≠ codorna; elos desatar / desastre / ufa; Valeu !!!',
    excerptEn:
      'Words: “nó” (Lat. *nodus*) — knot on the rope and in life; ≠ rope ≠ quail; links untying / disaster / phew; Valeu !!!',
    excerptEs:
      'Palabras: «nó» (lat. *nodus*) — nudo en la cuerda y en la vida; ≠ cuerda ≠ codorniz; vínculos desatar / desastre / ufa; ¡Valeu !!!',
