...
/**
 * Injeta / actualiza a inspeção-guia HC × seletividade (+ entradas no Guia).
 * Uso: node scripts/upsert-guia-hc-seletividade.js
 */
...
const post = buildGuiaHcSeletividadePost();
...
if (idx >= 0) {
  ...
  const after = posts.findIndex((p) => p.slug === 'inspecao-palavra-portaria');
...
let added = 0;
let updated = 0;
for (const item of GUIA_HC_SELETIVIDADE_GUIA_ITEMS) {
