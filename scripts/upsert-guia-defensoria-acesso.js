...
/**
 * Actualiza a inspeção-guia Defensoria Pública / acesso gratuito.
 * Uso: node scripts/upsert-guia-defensoria-acesso.js
 */
...
function upsertPost(post) {
  ...
  if (idx >= 0) {
    ...
    const after = posts.findIndex(
      (p) => p.slug === 'inspecao-guia-associacoes-pacientes'
    );
...
const post = buildGuiaDefensoriaAcessoPost();
upsertPost(post);
...
let added = 0;
let updated = 0;
for (const item of GUIA_DEFENSORIA_ACESSO_GUIA_ITEMS) {
