...
/**
 * Injeta / actualiza a inspeção do Padre Ticão em posts.json (+ overlay i18n).
 * Uso: node scripts/upsert-padre-ticao-inspecao.js
 */

const fs = require('fs');
const path = require('path');
const { buildPadreTicaoInspecaoPost } = require('../lib/pessoas-inspecoes-posts.js');

const ROOT = path.join(__dirname, '..');
const POSTS_FILE = path.join(ROOT, 'posts.json');
const I18N_FILE = path.join(ROOT, 'content', 'post-i18n.json');

const post = buildPadreTicaoInspecaoPost();
