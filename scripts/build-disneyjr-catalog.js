...
/**
 * Atualiza content/channels/disneyjrbr.json a partir do canal @disneyjrbr.
 * Catálogo recente via /videos + RSS; âncora editorial «Aqui no mar» (Pequena Sereia).
 * Uso: npm run build:disneyjr
 */
...
const OUT = path.join(ROOT, 'content', 'channels', 'disneyjrbr.json');

const CHANNEL_HANDLE = '@disneyjrbr';
const CHANNEL_URL = 'https://www.youtube.com/@disneyjrbr';
const CHANNEL_VIDEOS_URL = CHANNEL_URL + '/videos';
const FALLBACK_CHANNEL_ID = 'UCunkR_ekffRWZppVa1E6tIA';
const MAX_VIDEOS = 120;
const OEMBED_DELAY_MS = 120;
/** Vídeo âncora: Aqui no mar | A Pequena Sereia — elo desenhos × oceano × Moana. */
const SEED_VIDEO_ID = '0u1w_A8J6Hw';
...
async function resolveChannelId() {
  if (process.env.DISNEYJR_CHANNEL_ID) return process.env.DISNEYJR_CHANNEL_ID;
  try {
...
async function buildCatalog() {
  ...
  // Âncora editorial: «Aqui no mar» (Pequena Sereia) — desenhos × oceano
  if (!byId.has(SEED_VIDEO_ID)) {
    const seedTitle = await fetchOembedTitle(SEED_VIDEO_ID);
    byId.set(SEED_VIDEO_ID, {
      id: SEED_VIDEO_ID,
      title: seedTitle || 'Aqui no mar | com letra | A Pequena Sereia',
      url: 'https://www.youtube.com/watch?v=' + SEED_VIDEO_ID,
      note: 'Vídeo âncora BudGanja — desenho clássico oceano; elo Moana / mar / navegar'
    });
  } else {
    const emblem = byId.get(SEED_VIDEO_ID);
    byId.set(
      SEED_VIDEO_ID,
      Object.assign({}, emblem, {
        note:
          'Vídeo âncora BudGanja — desenho clássico oceano; elo Moana / mar / navegar'
      })
    );
  }
  videos = [...byId.values()];
  ...
  videos.sort((a, b) => {
    if (a.id === SEED_VIDEO_ID) return -1;
    if (b.id === SEED_VIDEO_ID) return 1;
  ...
  if (videos.length > MAX_VIDEOS) {
    const seed = videos.find((v) => v.id === SEED_VIDEO_ID);
    const rest = videos.filter((v) => v.id !== SEED_VIDEO_ID).slice(0, MAX_VIDEOS - 1);
    videos = seed ? [seed].concat(rest) : rest;
  }

  const next = Object.assign({}, catalog, {
    channelId,
    channelUrl: CHANNEL_URL,
    channelName: catalog.channelName || 'Disney Jr. Brasil',
    handle: CHANNEL_HANDLE,
    publishedAt: catalog.publishedAt || '2008-05-03',
    inspectedAt: new Date().toISOString(),
    videoCount: videos.length,
    mission:
      catalog.mission ||
      'Canal oficial Disney Jr. Brasil — desenhos, músicas e episódios infantis em português; no BudGanja entra como hub de Artes · desenho (elo Moana, oceano e literacia lúdica).',
  ...
  console.log('disneyjrbr.json:', videos.length, 'vídeos (canal', channelId + ', fonte: /videos + RSS)');
}

buildCatalog().catch((e) => {
  console.error('build:disneyjr falhou:', e.message);
  if (fs.existsSync(OUT)) {
