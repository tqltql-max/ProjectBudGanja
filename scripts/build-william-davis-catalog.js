...
/**
 * Atualiza content/channels/williamdavismd.json a partir do canal William Davis, MD.
 * Uso: node scripts/build-william-davis-catalog.js
 */
...
const OUT = path.join(ROOT, 'content', 'channels', 'williamdavismd.json');

const CHANNEL_HANDLE = '@williamdavismd';
const CHANNEL_URL = 'https://www.youtube.com/@williamdavismd';
const CHANNEL_VIDEOS_URL = CHANNEL_URL + '/videos';
const FALLBACK_CHANNEL_ID = 'UCZp4ONYOXZkf92UxxNnAiKQ';
      ...
      .get(
        url,
        {
          headers: Object.assign(
            { 'User-Agent': PAGE_UA, 'Accept-Language': 'en-US,en;q=0.9,pt;q=0.8' },
            headers
          )
        },
        (res) => {
...
function isLiveTitle(title) {
  return /transmissão ao vivo|ao vivo agora|live now/i.test(title || '');
}

async function resolveChannelId() {
  if (process.env.DAVIS_CHANNEL_ID) return process.env.DAVIS_CHANNEL_ID;
  const urls = [
    CHANNEL_URL,
    'https://www.youtube.com/c/drdavisinfinitehealth',
    'https://www.youtube.com/user/wheatbelly',
...
async function buildCatalog() {
  ...
  console.log('Aba /videos:', pageVideos.length);
  ...
  try {
    ...
    console.log('RSS:', rssVideos.length);
  } catch (e) {
    console.warn('RSS indisponível:', e.message);
  }
  ...
  const next = Object.assign({}, catalog, {
    ...
    channelName: 'William Davis, MD',
    handle: CHANNEL_HANDLE,
    website: 'https://drdavisinfinitehealth.com/',
    inspectedAt: new Date().toISOString(),
    videoCount: videos.length,
    mission:
      'Canal oficial do cardiologista William Davis (Wheat Belly / Infinite Health) — divulgação sobre trigo, microbioma, metabolismo e saúde preventiva.',
    videos
  });

  fs.mkdirSync(path.dirname(OUT), { recursive: true });
  fs.writeFileSync(OUT, JSON.stringify(next, null, 2) + '\n', 'utf8');
  console.log('williamdavismd.json:', videos.length, 'vídeos (canal', channelId + ')');
}

buildCatalog().catch((e) => {
  console.error('build william davis catalog falhou:', e.message);
  if (fs.existsSync(OUT)) {
