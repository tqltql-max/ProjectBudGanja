...
/**
 * Atualiza content/channels/movrecam.json a partir do canal MovReCam (aulas UNIFESP).
 * Uso: npm run build:movrecam
 */
...
const OUT = path.join(ROOT, 'content', 'channels', 'movrecam.json');

const CHANNEL_HANDLE = '@movrecam';
const CHANNEL_URL = 'https://www.youtube.com/c/MovReCam';
const CHANNEL_VIDEOS_URL = CHANNEL_URL + '/videos';
const FALLBACK_CHANNEL_ID = 'UCGvVEwpsxRlq18Uh2HYocbg';
...
async function resolveChannelId() {
  if (process.env.MOVRECAM_CHANNEL_ID) return process.env.MOVRECAM_CHANNEL_ID;
  const urls = [
    CHANNEL_URL,
    'https://www.youtube.com/' + CHANNEL_HANDLE,
    'https://www.youtube.com/movrecam'
  ];
...
async function buildCatalog() {
  ...
  const next = Object.assign({}, catalog, {
    ...
    website: catalog.website || 'https://movrecam.org.br/',
    contactEmail: catalog.contactEmail || 'curso.cannabismedicinal@unifesp.br',
    inspectedAt: new Date().toISOString(),
    videoCount: videos.length,
    mission:
      catalog.mission ||
      'Canal oficial do Movimento pela Regulamentação da Cannabis (MovReCam) — transmissão das aulas do curso de extensão UNIFESP sobre Cannabis sativa L. medicinal e conteúdo de advocacy em português.',
  ...
  console.log('movrecam.json:', videos.length, 'vídeos (canal', channelId + ', fonte: /videos + RSS)');
}

buildCatalog().catch((e) => {
  console.error('build:movrecam falhou:', e.message);
  if (fs.existsSync(OUT)) {
