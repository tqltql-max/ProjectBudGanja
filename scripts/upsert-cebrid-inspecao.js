...
/**
 * Injeta / actualiza a inspeção do CEBRID em posts.json (+ overlay i18n + SQL).
 * Uso: node scripts/upsert-cebrid-inspecao.js
 */

const fs = require('fs');
const path = require('path');
const { buildCebridInspecaoPost } = require('../lib/cebrid-inspecao-post.js');
...
function markSuggestionDone(slug) {
  ...
  const item = items.find((s) => s.id === 'cebrid-hub' || s.suggestedSlug === slug);
  if (!item) {
    console.warn('Sugestão cebrid-hub não encontrada');
    return;
  }
  item.status = 'feita';
...
async function main() {
  const post = buildCebridInspecaoPost();
