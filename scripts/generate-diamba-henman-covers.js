...
/** Capas 1200×630 — Diamba Sarabamba (Artes) + Anthony Henman (Pessoas). */
const fs = require('fs');
...
async function makeCover(outRel, eyebrow, title, subtitle, tone) {
  ...
  <text x="600" y="300" text-anchor="middle" font-family="Georgia, Times New Roman, serif" font-size="44" font-weight="700" fill="#f4fff8">${title}</text>
...
async function main() {
  await makeCover(
    'imagens/inspecoes/diamba-sarabamba-cover.jpg',
    'ARTES · LIVRO',
    'Diamba Sarabamba',
    '1986 · Ground · coletânea pioneira BR',
    ['#1a2820', '#2a3c28', '#0e1610']
  );
  await makeCover(
    'imagens/inspecoes/anthony-henman-cover.jpg',
    'PESSOAS · ANTROPÓLOGO',
    'Anthony Henman',
    'Tenetehara · diamba · Diamba Sarabamba',
