...
/** Capa 1200×630 para inspeção Pessoas · Gregorio Duvivier. */
const fs = require('fs');
...
async function main() {
  ...
  const outPath = path.join(ROOT, 'imagens', 'inspecoes', 'duvivier-cover.jpg');
  fs.mkdirSync(path.dirname(outPath), { recursive: true });

  const base = await sharp(heroPath)
    .rotate()
    .resize(1200, 630, { fit: 'cover', position: 'attention' })
    .modulate({ brightness: 0.66, saturation: 0.92 })
    .toBuffer();
  ...
  <text x="600" y="310" text-anchor="middle" font-family="Georgia, Times New Roman, serif" font-size="52" font-weight="700" fill="#fff8e0">Gregorio Duvivier</text>
  <text x="600" y="390" text-anchor="middle" font-family="Segoe UI, Arial, sans-serif" font-size="26" fill="#d7d7d7">Letras · O Céu da Língua · método da palavra</text>
</svg>`);
