...
/**
 * Atualiza content/channels/canabinall.json a partir do canal CANABinALL.
 * Uso: npm run build:canabinall
 */
...
const OUT = path.join(ROOT, 'content', 'channels', 'canabinall.json');

const CHANNEL_HANDLE = '@canabinall';
const CHANNEL_URL = 'https://www.youtube.com/@canabinall';
const CHANNEL_VIDEOS_URL = CHANNEL_URL + '/videos';
const FALLBACK_CHANNEL_ID = 'UCiACV4yFq6MoMknqCHaVlpw';
...
function isLiveTitle(title) {
  return /transmissão ao vivo|ao vivo agora/i.test(title || '');
}

async function resolveChannelId() {
  if (process.env.CANABINALL_CHANNEL_ID) return process.env.CANABINALL_CHANNEL_ID;
  const urls = [CHANNEL_URL, 'https://www.youtube.com/' + CHANNEL_HANDLE];
...
function extractVideosTab(data) {
  ...
  const tab =
    tabs.find((t) => /vídeo|video/i.test(String(t.tabRenderer?.title || ''))) || tabs[1];
...
async function buildCatalog() {
  ...
  const pageVideos = await fetchAllChannelVideos();
  ...
  const next = Object.assign({}, catalog, {
    ...
    channelName: catalog.channelName || 'CANABinALL',
    handle: CHANNEL_HANDLE,
    website: catalog.website || 'https://site.unifesp.br/cee/extensao/canabinall',
    contactEmail: catalog.contactEmail || 'e.rodrigues@unifesp.br',
    instagram: catalog.instagram || 'https://www.instagram.com/Canabinall/',
    inspectedAt: new Date().toISOString(),
    videoCount: videos.length,
    mission:
      catalog.mission ||
      'Projeto de extensão UNIFESP (CANABinALL) coordenado pela Profa. Dra. Eliana Rodrigues — divulgação científica semanal sobre fitocanabinoides, endocanabinoides e canabinoides sintéticos, em linguagem acessível, dedicado ao Prof. Emérito Elisaldo Carlini.',
    unifespCeeUrl: 'https://site.unifesp.br/cee/coordenacao',
    unifespCourseUrl: 'https://siex.siiu.unifesp.br/catalogo-siex/30063/mais-info',
    caecUrl:
      'https://site.unifesp.br/caec.diadema/programas-e-projetos/programas-e-projetos-no-campus/canabinall',
    videos
  });

  fs.mkdirSync(path.dirname(OUT), { recursive: true });
  fs.writeFileSync(OUT, JSON.stringify(next, null, 2), 'utf8');
  console.log('canabinall.json:', videos.length, 'vídeos (canal', channelId + ', fonte: /videos + RSS)');
}

buildCatalog().catch((e) => {
  console.error('build:canabinall falhou:', e.message);
  if (fs.existsSync(OUT)) {
