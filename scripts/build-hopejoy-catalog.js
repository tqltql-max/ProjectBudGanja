...
const { categorizeTitle, seriesOptionsFromVideos } = require('../lib/hopejoy-categories.js');

const ROOT = path.join(__dirname, '..');
const OUT = path.join(ROOT, 'content', 'channels', 'hopejoyoficial.json');

const CHANNEL_HANDLE = '@hopejoyoficial';
const CHANNEL_URL = 'https://www.youtube.com/@hopejoyoficial';
const CHANNEL_VIDEOS_URL = CHANNEL_URL + '/videos';
const FALLBACK_CHANNEL_ID = 'UC7UQZ9LI3DYvgcgkccWc2gg';
...
async function buildCatalog() {
  ...
  if (!channelId) {
    throw new Error('channelId HopeJoy não resolvido');
  }
  let pageVideos = [];
  try {
  ...
  const next = {
    ...
    channelName: 'Hope Joy',
    handle: CHANNEL_HANDLE,
    kickUrl: 'https://kick.com/hopejoyoficial',
    kickHandle: 'hopejoyoficial',
  ...
  console.log('hopejoyoficial.json:', videos.length, 'vídeos (canal', channelId + ')');
